report 50114 "Suggest Vendor Payments Dim"
{
    Caption = 'Suggest Vendor Payments Dim';
    ProcessingOnly = true;
    ApplicationArea = All;
    Permissions = tabledata "Vendor Ledger Entry"=rm,
        tabledata "Global Vendor Ledger Entry"=rm; //#179
        ...
        dataitem(Vendor; Vendor)
        {
            ...
            begin
                if UsePriority and not StopPayments then begin
                    ...
                    SetRange(Priority, 0);
                    if FindSet()then repeat Clear(VendorBalance);
                            CalcFields("Balance (LCY)");
                            VendorBalance:="Balance (LCY)";
                            if IncludeVendor(Vendor, VendorBalance)then begin
                                Window.Update(1, "No.");
                ...
                else if FindSet()then repeat ClearNegative();
                        until Next() = 0;
                ...
                //     repeat
                //         Message('%1.%2.%3', TempPayableVendorLedgerEntry."Company Code", TempPayableVendorLedgerEntry.Amount, TempPayableVendorLedgerEntry.Priority);
                //     until TempPayableVendorLedgerEntry.NEXT = 0;
                //Message('%1..totalcount', TempPayableVendorLedgerEntry.Count);
            ...
            begin
                if LastDueDateToPayReq = 0D then Error(Text000);
                if(PostingDate = 0D) and (not UseDueDateAsPostingDate)then Error(Text001);
                ...
                MessageText:='';
                if((BankPmtType = GenJnlLine2."Bank Payment Type"::" ") or SummarizePerVend) and (NextDocNo = '')then Error(Text002);
                if((BankPmtType = GenJnlLine2."Bank Payment Type"::"Manual Check") and not SummarizePerVend and not DocNoPerLine)then Error(Text017, GenJnlLine2.FieldCaption("Bank Payment Type"), Format(GenJnlLine2."Bank Payment Type"::"Manual Check"));
                if UsePaymentDisc and (LastDueDateToPayReq < WorkDate())then if not ConfirmManagement.GetResponseOrDefault(StrSubstNo(Text003, WorkDate()), true)then Error(Text005);
    ...
    requestpage
    {
                ...
                group(Options)
                {
                    ...
                    group("Find Payments")
                    {
                        ...
                        field(UseVendorPriority; UsePriority)
                        {
                            ...
                            trigger OnValidate()
                            begin
                                if not UsePriority and (AmountAvailable <> 0)then Error(Text011);
                            end;
                        }
                        field("Available Amount (LCY)"; AmountAvailable)
                        {
                            ...
                            trigger OnValidate()
                            begin
                                if AmountAvailable <> 0 then UsePriority:=true;
                            end;
                        }
                    ...
                    group("Summarize Results")
                    {
                        ...
                        field(SummarizePerVendor; SummarizePerVend)
                        {
                            ...
                            trigger OnValidate()
                            begin
                                if SummarizePerVend and UseDueDateAsPostingDate then Error(PmtDiscUnavailableErr);
                            end;
                        }
                    ...
                    group("Fill in Journal Lines")
                    {
                        ...
                        field(UseDueDateAsPostingDate; UseDueDateAsPostingDate)
                        {
                            ...
                            trigger OnValidate()
                            begin
                                if UseDueDateAsPostingDate and SummarizePerVend then Error(PmtDiscUnavailableErr);
                                if not UseDueDateAsPostingDate then Clear(DueDateOffset);
                            end;
                        }
                        ...
                        field(NewDocNoPerLine; DocNoPerLine)
                        {
                            ...
                            trigger OnValidate()
                            begin
                                if not UsePriority and (AmountAvailable <> 0)then Error(Text013);
                            end;
                        }
                        ...
                        field(BalAccountNo; GenJnlLine2."Bal. Account No.")
                        {
                            ...
                            trigger OnLookup(var Text: Text): Boolean begin
                                ...
                                GenJnlLine2."Bal. Account Type"::Customer, GenJnlLine2."Bal. Account Type"::Vendor: Error(Text009, GenJnlLine2.FieldCaption("Bal. Account Type"));
                        ...
                        field(BankPaymentType; GenJnlLine2."Bank Payment Type")
                        {
                            ...
                            trigger OnValidate()
                            begin
                                if(GenJnlLine2."Bal. Account Type" <> GenJnlLine2."Bal. Account Type"::"Bank Account") and (GenJnlLine2."Bank Payment Type".AsInteger() > 0)then Error(Text010, GenJnlLine2.FieldCaption("Bank Payment Type"), GenJnlLine2.FieldCaption("Bal. Account Type"));
                            end;
                        }
                    }
                    group(Control22)
        ...
        begin
            ...
            SummarizePerVend:=true; //Sgarg - Added
        end;
        trigger OnOpenPage()
        begin
            if LastDueDateToPayReq = 0D then LastDueDateToPayReq:=WorkDate();
    ...
    begin
        if EnvironmentInfo.IsSaaS()then CheckOtherJournalBatches:=true;
    end;
    ...
    begin
        Commit();
        if not TempVendorLedgerEntry.IsEmpty()then if ConfirmManagement.GetResponse(Text024, true)then PAGE.RunModal(0, TempVendorLedgerEntry);
        if CheckOtherJournalBatches then if not TempErrorMessage.IsEmpty()then if ConfirmManagement.GetResponse(ReviewNotSuggestedLinesQst, true)then TempErrorMessage.ShowErrorMessages(false);
    end;
    trigger OnPreReport()
    begin
    ...
    var dd: Report "Suggest Vendor Payments";
    Vend2: Record Vendor;
    ...
    NoofPaymentBuffer: Integer; //#179 TEC.VJ
    LineNo: Integer; //#179 TEC.VJ
    ...
    begin
        if not GenJnlBatch.Get(GenJnlLine."Journal Template Name", GenJnlLine."Journal Batch Name")then GenJnlBatch.Get(JnlTemplateName, JnlBatchName)
        else
        begin
        ...
        begin
            ...
            NextBankDocNo:=NoSeries.PeekNextNo(l_GLSetup."Bank Document Nos.", PostingDate);
        //#179 TEC.VJ<<
        end;
        //#234 VJ 21Feb2025
        l_GenJnlLine.Reset();
    ...
    begin
        ...
        VendLedgEntry.Reset();
        VendLedgEntry.SetCurrentKey("Company Name", "Vendor No.", Open, Positive, "Due Date");
        //Sgarg >>
        IF RecCompMap."Central payment company" <> '' then VendLedgEntry.SetFilter("Company Name", RecCompMap."Central payment company")
        else
        ...
        if SkipExportedPayments then VendLedgEntry.SetRange("Exported to Payment File", false);
        ...
        //NS <<
        IF NOT(ShortcutDim3 = '')then VendLedgEntry.SetFilter("Shortcut Dimension 3 Code_PB", ShortcutDim3);
        IF NOT(ShortcutDim4 = '')then VendLedgEntry.SetFilter("Shortcut Dimension 4 Code_PB", ShortcutDim4);
        IF NOT(ShortcutDim5 = '')then VendLedgEntry.SetFilter("Shortcut Dimension 5 Code_PB", ShortcutDim5);
        IF NOT(ShortcutDim6 = '')then VendLedgEntry.SetFilter("Shortcut Dimension 6 Code_PB", ShortcutDim6);
        IF NOT(ShortcutDim7 = '')then VendLedgEntry.SetFilter("Shortcut Dimension 7 Code_PB", ShortcutDim7);
        IF NOT(ShortcutDim8 = '')then VendLedgEntry.SetFilter("Shortcut Dimension 8 Code_PB", ShortcutDim8);
        IF NOT(ShortcutDim9 = '')then VendLedgEntry.SetFilter("Shortcut Dimension 9 Code_PB", ShortcutDim9);
        IF NOT(ShortcutDim10 = '')then VendLedgEntry.SetFilter("Shortcut Dimension 10 Code_PB", ShortcutDim10);
        IF NOT(ShortcutDim11 = '')then VendLedgEntry.SetFilter("Shortcut Dimension 11 Code_PB", ShortcutDim11);
        IF NOT(ShortcutDim12 = '')then VendLedgEntry.SetFilter("Shortcut Dimension 12 Code_PB", ShortcutDim12);
        IF NOT(ShortcutDim13 = '')then VendLedgEntry.SetFilter("Shortcut Dimension 13 Code_PB", ShortcutDim13);
        //NS >>
        // OnGetVendLedgEntriesOnAfterVendLedgEntrySetFilters(VendLedgEntry, Vendor, PostingDate, LastDueDateToPayReq, Positive, Future);
        if VendLedgEntry.FindSet()then repeat IsHandled:=false;
                ...
                if not IsHandled then begin
                    ...
                    if VendLedgEntry."Accepted Pmt. Disc. Tolerance" or (VendLedgEntry."Accepted Payment Tolerance" <> 0)then begin
                        ...
                        CODEUNIT.Run(CODEUNIT::"Global Vend. Entry-Edit", VendLedgEntry);
                    end;
                end;
    ...
    local procedure IncludeVendor(Vendor: Record Vendor; VendorBalance: Decimal)Result: Boolean begin
    ...
    begin
        ...
        GenJnlLine."Invoice Link":=VendLedgEntry."Invoice Link";
        GenJnlLine.Validate(Amount);
        if UsePriority then TempPayableVendorLedgerEntry.Priority:=Vendor.Priority
        else
        ...
        TempPayableVendorLedgerEntry."Company Code":=VendLedgEntry."Company Code"; //Sgarg - Added
    ...
    begin
        ...
        if TempPayableVendorLedgerEntry.FindSet()then begin
            repeat if TempPayableVendorLedgerEntry."Currency Code" <> PrevCurrency then begin
                ...
                if(OriginalAmtAvailable = 0) or (AmountAvailable >= CurrencyBalance + TempPayableVendorLedgerEntry."Amount (LCY)")then CurrencyBalance:=CurrencyBalance + TempPayableVendorLedgerEntry."Amount (LCY)"
                else
    ...
    var
        ...
        L_VLE: Record "Vendor Ledger Entry"; //TEC-SGarg
    begin
        ...
        Clear(LineNo);
        NoofPaymentBuffer:=TempPayableVendorLedgerEntry.Count; //TEC.VJ
        if TempPayableVendorLedgerEntry.Find('-')then //Message('%1..Payable Count', TempPayableVendorLedgerEntry);
            repeat TempPayableVendorLedgerEntry.SetRange("Vendor No.", TempPayableVendorLedgerEntry."Vendor No.");
                ...
                repeat LineNo+=1; //TEC.VJ
                    ...
                    VendLedgEntry.Get(TempPayableVendorLedgerEntry."Vendor Ledg. Entry No.", TempPayableVendorLedgerEntry."Company Code"); //SGarg- Added
                    ...
                    if HandledEntry then begin
                        ...
                        //Sgarg Added >>
                        TempVendorPaymentBuffer."Creditor No.":=VendLedgEntry."Creditor No.";
                        TempVendorPaymentBuffer."Payment Reference":=VendLedgEntry."Payment Reference";
                        TempVendorPaymentBuffer."Exported to Payment File":=VendLedgEntry."Exported to Payment File";
                        TempVendorPaymentBuffer."Applies-to Ext. Doc. No.":=VendLedgEntry."External Document No.";
                        TempVendorPaymentBuffer."Vendor Posting Group":=VendLedgEntry."Vendor Posting Group";
                        TempVendorPaymentBuffer."Remit-to Code":=VendLedgEntry."Remit-to Code"; //- Standard code commented
                        //TempVendorPaymentBuffer."Remit-to Code" := VendLedgEntry."Company Name"; //- my code commented
                        //Sgarg Added <<
                        ...
                        if IsNotAppliedEntry(GenJnlLine, VendLedgEntry)then begin
                            if SummarizePerVend then begin
                                ...
                                begin
                                    ...
                                    TempVendorPaymentBuffer."Bank Document No. Applied":=NextBankDocNo; //#179 VJ 22012025
                                    if DocNoPerLine then RunIncrementDocumentNo(true);
                            ...
                            //IF VendLedgEntry."Company Name" = company
                            end
                            else
                            begin
                        ...
                        VendLedgEntry."Bank Document No. Applied":=TempVendorPaymentBuffer."Bank Document No. Applied"; //#179 VJ 22Jan2025
                        //TEC-SGarg- Added>>
                        if UpperCase(VendLedgEntry."Company Name") = UpperCase(CompanyName)then begin
                            L_VLE.Get(VendLedgEntry."Entry No.");
                            L_VLE."Applies-to ID":=VendLedgEntry."Applies-to ID";
                            L_VLE."Accepted Pmt. Disc. Tolerance":=VendLedgEntry."Accepted Pmt. Disc. Tolerance";
                            L_VLE."Accepted Payment Tolerance":=VendLedgEntry."Accepted Payment Tolerance";
                            L_VLE."Amount to Apply":=VendLedgEntry."Amount to Apply";
                            L_VLE."Applying Entry":=VendLedgEntry."Applying Entry";
                            L_VLE."Bank Document No. Applied":=VendLedgEntry."Bank Document No. Applied"; //#179 VJ 22Jan2025
                            L_VLE.Modify();
                        END;
                        //TEC-SGarg- Added<<
                        CODEUNIT.Run(CODEUNIT::"Global Vend. Entry-Edit", VendLedgEntry);
                    ...
                    begin
                        TempVendorLedgerEntry:=VendLedgEntry; //Chacha
        ...
        NoofPaymentBuffer:=0; //#179 TEC.VJ
        Clear(LineNo);
        NoofPaymentBuffer:=TempVendorPaymentBuffer.Count; //#179 TEC.VJ
        if TempVendorPaymentBuffer.Find('-')then repeat LineNo:=LineNo + 1; //#179 TEC.VJ
    ...
    begin
        ...
        if SummarizePerVend then begin
            ...
            GenJnlLine."Bank Document No.":=TempVendorPaymentBuffer."Bank Document No. Applied" //TEC.VJ 23012025
 end
        else if DocNoPerLine then begin
                if TempVendorPaymentBuffer.Amount < 0 then GenJnlLine."Document Type":=GenJnlLine."Document Type"::Refund;
                GenJnlLine."Document No.":=NextDocNo;
                GenJnlLine."Bank Document No.":=NextBankDocNo; //TEC.VJ 23012025
                RunIncrementDocumentNo(false);
            end
            else if(GenJnlLine2."Bal. Account No." = '') and not DocNoPerLine then GenJnlLine."Document No.":=NextDocNo
                else if(TempVendorPaymentBuffer."Vendor No." = TempOldVendorPaymentBuffer."Vendor No.") and (TempVendorPaymentBuffer."Currency Code" = TempOldVendorPaymentBuffer."Currency Code")then begin
                        GenJnlLine."Document No.":=TempOldVendorPaymentBuffer."Document No.";
                        GenJnlLine."Bank Document No.":=TempVendorPaymentBuffer."Bank Document No."; //TEC.VJ 23012025
                    end
                    else
                    begin
                        GenJnlLine."Document No.":=NextDocNo;
        ...
        ShowPostingDateWarning:=ShowPostingDateWarning or SetPostingDate(GenJnlLine, GetVendorLedgerEntryDueDate(TempVendorPaymentBuffer."Vendor Ledg. Entry No."), PostingDate);
        GenJnlLine.Validate("Account No.", TempVendorPaymentBuffer."Vendor No.");
        Vendor.Get(TempVendorPaymentBuffer."Vendor No.");
        if(Vendor."Pay-to Vendor No." <> '') and (Vendor."Pay-to Vendor No." <> GenJnlLine."Account No.")then Message(Text025, Vendor.TableCaption(), Vendor."No.", Vendor.FieldCaption("Pay-to Vendor No."), Vendor."Pay-to Vendor No.");
        ...
        if SummarizePerVend then GenJnlLine."Applies-to ID":=GenJnlLine."Document No.";
        ...
        // GenJnlLine."Company Code" := TempVendorPaymentBuffer."Remit-to Code";  //SGarg - added
        GenJnlLine."Remit-to Code":=TempVendorPaymentBuffer."Remit-to Code"; //Sgarg - Commented - standard code
        //Sgarg - Added>>
        GenJnlLine."Creditor No.":=TempVendorPaymentBuffer."Creditor No.";
        GenJnlLine."Payment Reference":=TempVendorPaymentBuffer."Payment Reference";
        GenJnlLine."Exported to Payment File":=TempVendorPaymentBuffer."Exported to Payment File";
        GenJnlLine."Applies-to Ext. Doc. No.":=TempVendorPaymentBuffer."Applies-to Ext. Doc. No.";
        GenJnlLine."Posting Group":=TempVendorPaymentBuffer."Vendor Posting Group";
        GenJnlLine."Bypass API":=true; //TEC.VJ 17DEC2024
        GenJnlLine."Suggest Vendor Line":=true; //TEC.VJ 17DEC2024
        //genjnl
        // CommonFunc.CreateBankDocuemntNo(GenJnlLine);//TEC.VJ 17DEC2024
        //GenJnlLine."Remit-to Code" := "Remit-to Code";
        //TempVendorPaymentBuffer.CopyFieldsToGenJournalLine(GenJnlLine); 
        //Sgarg - Added<<
        //TempPaymentBuffer.CopyFieldsFromVendorPaymentBuffer(TempVendorPaymentBuffer);
        // TempVendorPaymentBuffer.CopyFieldsFromPaymentBuffer(TempPaymentBuffer);
        // GenJnlLine."Bank Document No." := TempVendorPaymentBuffer."Bank Document No. Applied";//#179 TEC.VJ
        GlobalGenJnl.UpdateAppliedAmountForVendor(GenJnlLine); //TEC.VJ 23012025
    ...
    begin
        ...
        //#179 TEC.VJ>>
        if NoofPaymentBuffer <> LineNo then NextBankDocNo:=NoSeriesBatch.GetNextNo(l_GLSetup."Bank Document Nos.", GenJnlLine."Posting Date", true);
        if(LineNo = 1) and (PrepareBuffer = true)then NextBankDocNo:=NoSeriesBatch.GetNextNo(l_GLSetup."Bank Document Nos.", GenJnlLine."Posting Date", true);
    //#179 TEC.VJ<<
    end;
    ...
    begin
        ...
        if NewDimensionID <> GenJnlLine."Dimension Set ID" then begin
            ...
            GenJnlLine."Dimension Set ID":=DimMgt.GetCombinedDimensionSetID(DimSetIDArr, GenJnlLine."Shortcut Dimension 1 Code", GenJnlLine."Shortcut Dimension 2 Code");
        end;
        if SummarizePerVend then begin
            DimMgt.GetDimensionSet(TempDimSetEntry, GenJnlLine."Dimension Set ID");
            if AdjustAgainstSelectedDim(TempDimSetEntry, TempDimSetEntry2)then GenJnlLine."Dimension Set ID":=DimMgt.GetDimensionSetID(TempDimSetEntry2);
    ...
    begin
        if BalAccType = BalAccType::"Bank Account" then if BalAccNo <> '' then begin
                ...
                if BankAcc."Currency Code" <> '' then TempPayableVendorLedgerEntry.SetRange("Currency Code", BankAcc."Currency Code");
            end;
    end;
    ...
    begin
        if BalAccType = BalAccType::"Bank Account" then if BalAccNo <> '' then begin
                ...
                if BankAcc."Currency Code" <> '' then begin
                    ...
                    if TmpTempPayableVendorLedgerEntry.FindSet()then repeat TempPayableVendorLedgerEntry2:=TmpTempPayableVendorLedgerEntry;
                    ...
                    if SeveralCurrencies then MessageText:=StrSubstNo(Text020, BankAcc.FieldCaption("Currency Code"), BankAcc."Currency Code")
                    else
                        MessageText:=StrSubstNo(Text021, BankAcc.FieldCaption("Currency Code"), BankAcc."Currency Code");
                end
                else
                    MessageText:=Text022;
            end;
    ...
    begin
        ...
        while TempPayableVendorLedgerEntry.Next() <> 0 do begin
            ...
            if TempCurrency.Insert()then begin
                ...
                repeat CurrencyBalance:=CurrencyBalance + TempPayableVendorLedgerEntry."Amount (LCY)" until TempPayableVendorLedgerEntry.Next() = 0;
    ...
    local procedure GetMessageToRecipient(SummarizePerVend: Boolean): Text[140]var
        VendorLedgerEntry: Record "Vendor Ledger Entry";
        IsHandled: Boolean;
        Message: Text[140];
    begin
        // OnBeforeGetMessageToRecipient(SummarizePerVend, TempVendorPaymentBuffer, IsHandled, Message);
        if IsHandled then exit(Message);
        if SummarizePerVend then exit(CompanyInformation.Name);
    ...
    local procedure SetPostingDate(var GenJnlLine: Record "Gen. Journal Line"; DueDate: Date; PostingDate: Date): Boolean begin
        if not UseDueDateAsPostingDate then begin
            GenJnlLine.Validate("Posting Date", PostingDate);
            exit(false);
        end;
        if DueDate = 0D then DueDate:=GetDueDateOrPaymentDiscountDate(GenJnlLine.GetAppliesToDocDueDate(), GenJnlLine.GetAppliesToDocPmtDiscountDate(), UsePaymentDisc and (GenJnlLine."Payment Discount %" <> 0));
        exit(GenJnlLine.SetPostingDateAsDueDate(DueDate, DueDateOffset));
    end;
    local procedure GetVendorLedgerEntryDueDate(VendorLedgerEntryNo: Integer): Date var
        FromVendorLedgerEntry: Record "Vendor Ledger Entry";
    begin
        if FromVendorLedgerEntry.Get(VendorLedgerEntryNo)then exit(GetDueDateOrPaymentDiscountDate(FromVendorLedgerEntry."Due Date", FromVendorLedgerEntry."Pmt. Discount Date", UsePaymentDisc and (FromVendorLedgerEntry."Original Pmt. Disc. Possible" <> 0)));
        exit(PostingDate);
    end;
    local procedure GetDueDateOrPaymentDiscountDate(DueDate: Date; PmtDiscountDate: Date; PmtDiscountPossible: Boolean): Date begin
        if not PmtDiscountPossible or (PmtDiscountDate = 0D)then exit(DueDate);
        if PmtDiscountDate < DueDate then exit(PmtDiscountDate);
        exit(DueDate);
    end;
    ...
    begin
        ...
        if SummarizePerDim then begin
            DimBuf.Reset();
            DimBuf.DeleteAll();
            if SelectedDim.FindSet()then repeat if DimSetEntry.Get(VendLedgEntry."Dimension Set ID", SelectedDim."Dimension Code")then InsertDimBuf(DimBuf, DATABASE::"Dimension Buffer", 0, DimSetEntry."Dimension Code", DimSetEntry."Dimension Value Code");
                until SelectedDim.Next() = 0;
            ...
            if TempVendorPaymentBuffer."Dimension Entry No." <> 0 then begin
                GLSetup.Get();
                if DimCodeIsInDimBuf(GLSetup."Global Dimension 1 Code", DimBuf)then TempVendorPaymentBuffer."Global Dimension 1 Code":=VendLedgEntry."Global Dimension 1 Code"
                else
                    TempVendorPaymentBuffer."Global Dimension 1 Code":='';
                if DimCodeIsInDimBuf(GLSetup."Global Dimension 2 Code", DimBuf)then TempVendorPaymentBuffer."Global Dimension 2 Code":=VendLedgEntry."Global Dimension 2 Code"
                else
    ...
    // end;
    local procedure IsNotAppliedEntry(GenJournalLine: Record "Gen. Journal Line"; VendorLedgerEntry: Record "Global Vendor Ledger Entry"): Boolean begin
        if not IsNotAppliedToCurrentBatchLine(GenJournalLine, VendorLedgerEntry)then exit(false);
        exit(IsNotAppliedToOtherBatchLine(GenJournalLine, VendorLedgerEntry));
    end;
    ...
    // end;
    local procedure IsNotAppliedToCurrentBatchLine(GenJournalLine: Record "Gen. Journal Line"; VendorLedgerEntry: Record "Global Vendor Ledger Entry"): Boolean var
        PaymentGenJournalLine: Record "Gen. Journal Line";
    begin
        PaymentGenJournalLine.SetRange("Journal Template Name", GenJournalLine."Journal Template Name");
        PaymentGenJournalLine.SetRange("Journal Batch Name", GenJournalLine."Journal Batch Name");
    ...
    //     end;
    // end;
    local procedure IsNotAppliedToOtherBatchLine(GenJournalLine: Record "Gen. Journal Line"; VendorLedgerEntry: Record "Global Vendor Ledger Entry"): Boolean var
        PaymentGenJournalLine: Record "Gen. Journal Line";
    begin
        if not CheckOtherJournalBatches then exit(true);
        PaymentGenJournalLine.SetRange("Document Type", PaymentGenJournalLine."Document Type"::Payment);
        PaymentGenJournalLine.SetRange("Account Type", PaymentGenJournalLine."Account Type"::Vendor);
        PaymentGenJournalLine.SetRange("Account No.", VendorLedgerEntry."Vendor No.");
        PaymentGenJournalLine.SetRange("Applies-to Doc. Type", VendorLedgerEntry."Document Type");
        PaymentGenJournalLine.SetRange("Applies-to Doc. No.", VendorLedgerEntry."Document No.");
        if PaymentGenJournalLine.IsEmpty()then exit(true);
        if PaymentGenJournalLine.FindSet()then begin
            repeat if(PaymentGenJournalLine."Journal Batch Name" <> GenJournalLine."Journal Batch Name") or (PaymentGenJournalLine."Journal Template Name" <> GenJournalLine."Journal Template Name")then LogNotSuggestedPaymentMessage(PaymentGenJournalLine);
            until PaymentGenJournalLine.Next() = 0;
            exit(TempErrorMessage.IsEmpty);
        end;
    end;
    ...
    begin
        TempErrorMessage.LogMessage(GenJournalLine, GenJournalLine.FieldNo("Applies-to ID"), TempErrorMessage."Message Type"::Warning, StrSubstNo(NotSuggestedPaymentInfoTxt, GenJournalLine.FieldCaption("Journal Template Name"), GenJournalLine."Journal Template Name", GenJournalLine.FieldCaption("Journal Batch Name"), GenJournalLine."Journal Batch Name", GenJournalLine.FieldCaption("Applies-to Doc. No."), GenJournalLine."Applies-to Doc. No."));
    end;
