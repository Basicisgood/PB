codeunit 50201 "Concur Get Attendee Info"
{ //PS009
    trigger OnRun()
    begin
    //GetIdentity(true);
    ...
    procedure GetAttendeeInfo(Process: Boolean; AttendeeID: text): Text var
        TokenUrl: Text[1024];
        HttpClient: HttpClient;
        ResponseMessage: HttpResponseMessage;
        RequestHeader: HttpHeaders;
    ...
    begin
        APISetup.GET;
        APISetup.TestField("Is Enable", true);
        APISetup.TestField("Get Attendee Info URL");
        AccessToken:=GenerateRefreshToken();
        IF AccessToken = '' then Error('No Access Token generated');
        HttpClient.DefaultRequestHeaders().Add('Authorization', AccessToken);
        HttpClient.Get(APISetup."Get Attendee Info URL" + AttendeeID, ResponseMessage);
        IF ResponseMessage.IsSuccessStatusCode()THEN BEGIN
            ...
            if Process then begin
                ...
                //InsertAPILog;
                LogEntryNo:=ConcurAPIInboundLog.InsertLog(7, JsonResponseTxt);
                AttendeeStaging.reset;
                AttendeeStaging.SetRange(AttendeeID, AttendeeID);
                if AttendeeStaging.FindSet()then repeat AttendeeStaging.AttendeeTypeCode:=ResponseJsonHeader(3, 'items[0].attendeeTypeCode');
                        AttendeeStaging.FirstName:=ResponseJsonHeader(3, 'items[0].firstName');
                        AttendeeStaging.LastName:=ResponseJsonHeader(3, 'items[0].lastName');
                        AttendeeStaging.MiddleInitial:=ResponseJsonHeader(3, 'items[0].middleInitial');
                        AttendeeStaging.Suffix:=ResponseJsonHeader(3, 'items[0].suffix');
                        AttendeeStaging.Company:=ResponseJsonHeader(3, 'items[0].company');
                        AttendeeStaging.Title:=ResponseJsonHeader(3, 'items[0].title');
                        AttendeeStaging.ExternalId:=ResponseJsonHeader(3, 'items[0].externalId');
                        if ResponseJsonHeader(3, 'items[0].hasExceptionsPrevYear') = 'No' then AttendeeStaging.HasExceptionsPrevYear:=false
                        else
                            AttendeeStaging.HasExceptionsPrevYear:=true;
                        if ResponseJsonHeader(3, 'items[0].hasExceptionsYtd') = 'No' then AttendeeStaging.HasExceptionsYtd:=false
                        else
                            AttendeeStaging.HasExceptionsYtd:=true;
                        Evaluate(AttendeeStaging.TotalAmountPrevYear, ResponseJsonHeader(3, 'items[0].totalAmountPrevYear'));
                        Evaluate(AttendeeStaging.TotalAmountYtd, ResponseJsonHeader(3, 'items[0].totalAmountYtd'));
                        Evaluate(AttendeeStaging.VersionNumber, ResponseJsonHeader(3, 'items[0].versionNumber'));
                        AttendeeStaging.OwnerName:=ResponseJsonHeader(3, 'items[0].ownerName');
                        AttendeeStaging.OwnerUserId:=ResponseJsonHeader(3, 'items[0].ownerUserId');
                        AttendeeStaging.CurrencyCode:=ResponseJsonHeader(3, 'items[0].currencyCode');
                        AttendeeStaging.Modify();
                    until AttendeeStaging.Next() = 0;
    ...
    LogEntryNo: Integer;
    AttendeeStaging: Record "Concur inbound Attende Staging";
}
