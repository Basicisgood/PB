page 50199 "Applied GL Entries PB"
{
    ...
    begin
        ...
        if SelectedOneDetailedGLEntryCZA.FindSet()then repeat if SelectedOneDetailedGLEntryCZA."G/L Entry No." = SelectedOneDetailedGLEntryCZA."Applied G/L Entry No." then begin
                    ...
                    if SelectedTwoDetailedGLEntryCZA.FindSet()then repeat if SelectedTwoDetailedGLEntryCZA."G/L Entry No." <> SelectedTwoDetailedGLEntryCZA."Applied G/L Entry No." then if Rec.Get(SelectedTwoDetailedGLEntryCZA."G/L Entry No.")then Rec.Mark(true);
                        until SelectedTwoDetailedGLEntryCZA.Next() = 0;
                end
                else if Rec.Get(SelectedOneDetailedGLEntryCZA."Applied G/L Entry No.")then Rec.Mark(true);
            until SelectedOneDetailedGLEntryCZA.Next() = 0;
    end;
    procedure Caption(): Text var
        CaptionTok: Label '%1 %2', Locked = true;
    begin
        exit(StrSubstNo(CaptionTok, GLAccount."No.", GLAccount.Name));
    end;
}
