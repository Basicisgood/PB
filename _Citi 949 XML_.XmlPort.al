xmlport 50113 "Citi 949 XML"
{
    ...
    trigger OnBeforePassVariable()
    begin
        fininstnid_othr_id:=CITI."Word Link";
        if fininstnid_othr_id = '' then currXMLport.Skip();
    end;
    }
    }
    }
    ...
    begin
        if cdttrftxinf."Creditor Swift Code" = '' then currXMLport.Skip();
    end;
    }
    ...
    fieldelement(mmbid;
    cdttrftxinf."Creditor IFSC Code")
    {
    MinOccurs = Zero;
    XmlName = 'MmbId';
    }
    }
    textelement(cdtragt_PstlAdr)
    {
    XmlName = 'PstlAdr';
    ...
    /*   trigger OnBeforePassVariable()
                              begin
                                  if cdttrftxinf."Creditor Swift Code" = '' then
                                      currXMLport.Skip();
                              end; */
    }
    textelement(cdtr)
    ...
    trigger OnBeforePassField()
    begin
        if cdttrftxinf."Creditor Address 3" = '' then currXMLport.Skip();
    end;
    }
