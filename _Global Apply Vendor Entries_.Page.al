page 50192 "Global Apply Vendor Entries"
{
    ...
    layout
    {
        area(content)
        {
            ...
            repeater(Control1)
            {
                ...
                // Editable = RepeaterEnable;//TEC.vj
                field(AppliesToID; Rec."Applies-to ID")
                {
                    ...
                    trigger OnValidate()
                    begin
                        Error(StrSubstNo('You cannot modify %1 value.', Rec.FieldCaption("Applies-to ID"))); //#253 TEC.VJ 10FEB2025
                        if(CalcType = CalcType::"Gen. Jnl. Line") and (ApplnType = ApplnType::"Applies-to Doc. No.")then Error(CannotSetAppliesToIDErr);
                        SetVendApplId(true);
                        if Rec."Applies-to ID" <> '' then UpdateCustomAppliesToIDForGenJournal(Rec."Applies-to ID");
                ...
                field("Bank Document No. Applied"; Rec."Bank Document No. Applied")
                {
                    ...
                    trigger OnValidate()
                    begin
                        Error(StrSubstNo('You cannot modify %1 value.', Rec.FieldCaption("Bank Document No. Applied"))); //#253 TEC.VJ 10FEB2025
                    end;
                }
                ...
                field("Amount to Apply"; Rec."Amount to Apply")
                {
                    ...
                    trigger OnValidate()
                    begin
                        CODEUNIT.Run(CODEUNIT::"Global Vend. Entry-Edit", Rec);
                        ...
                        Rec.Get(Rec."Entry No.", rec."Company Name");
            ...
            group("&Application")
            {
                ...
                action("Show Only Selected Entries to Be Applied")
                {
                    ...
                    begin
                        ...
                        if ShowAppliedEntries then if CalcType = CalcType::"Gen. Jnl. Line" then Rec.SetRange("Applies-to ID", GenJnlLine."Applies-to ID")
                            else
                            begin
    ...
    // RepeatEditable();//TEC.VJ
    end;
    trigger OnAfterGetRecord()
    begin
        StyleTxt:=Rec.SetStyle();
    // RepeatEditable()//TEC.VJ 24DEC2024
    end;
    trigger OnInit()
    begin
    ...
    begin
        ...
        if not IsHandled then CODEUNIT.Run(CODEUNIT::"Global Vend. Entry-Edit", Rec);
        if Rec."Applies-to ID" <> xRec."Applies-to ID" then CalcApplnAmount();
        exit(false);
    end;
    //TEC.VJ>>
    // local procedure RepeatEditable()
    // begin
    //     if Rec."Document No." <> Rec."Applies-to ID" then
    //         RepeaterEnable := false
    //     else
    //         RepeaterEnable := true;
    // end;
    //TEC.VJ<<
    local procedure JournalHasDocumentNo(AppliesToIDCode: Code[50]): Boolean var
    ...
    begin
        ...
        GLSetup.Get();
        if CalcType = CalcType::"Gen. Jnl. Line" then CalcApplnAmount();
        PostingDone:=false;
            ...
            if ApplnType = ApplnType::"Applies-to Doc. No." then begin
                if OK then begin
                    ...
                    if RaiseError then begin
                        OK:=false;
                        Error(EarlierPostingDateErr, TempApplyingVendLedgEntry."Document Type", TempApplyingVendLedgEntry."Document No.", Rec."Document Type", Rec."Document No.");
                    end;
    ...
    protected var Currency: Record Currency;
    CurrExchRate: Record "Currency Exchange Rate";
    TempApplyingVendLedgEntry: Record "Global Vendor Ledger Entry" temporary;
    ...
    begin
        ...
        case ApplnTypeSelect of GenJnlLine.FieldNo("Applies-to Doc. No."): ApplnType:=ApplnType::"Applies-to Doc. No.";
        GenJnlLine.FieldNo("Applies-to ID"): ApplnType:=ApplnType::"Applies-to ID";
        end;
        SetApplyingVendLedgEntry();
    end;
    internal procedure GetCustomAppliesToID(): Code[50]begin
        if TimesSetCustomAppliesToID <> 1 then exit('');
        exit(CustomAppliesToID);
    end;
    local procedure UpdateCustomAppliesToIDForGenJournal(NewAppliesToID: Code[50])
    begin
        if(not GenJnlLineApply) or (ApplnType <> ApplnType::"Applies-to ID")then exit;
        if JournalHasDocumentNo(NewAppliesToID)then exit;
        if(CustomAppliesToID = '') or ((CustomAppliesToID <> '') and (CustomAppliesToID <> NewAppliesToID))then TimesSetCustomAppliesToID+=1;
        CustomAppliesToID:=NewAppliesToID;
    end;
    local procedure RunVendEntryEdit(var VendorLedgerEntry: Record "Global Vendor Ledger Entry")
    begin
        OnBeforeRunVendEntryEdit(VendorLedgerEntry);
        CODEUNIT.Run(CODEUNIT::"Global Vend. Entry-Edit", VendorLedgerEntry);
    end;
    procedure SetPurch(NewPurchHeader: Record "Purchase Header"; var NewVendLedgEntry: Record "Global Vendor Ledger Entry"; ApplnTypeSelect: Integer)
    begin
        ...
        case PurchHeader."Document Type" of PurchHeader."Document Type"::"Return Order", PurchHeader."Document Type"::"Credit Memo": ApplyingAmount:=TotalPurchLine."Amount Including VAT"
        else
        ...
        case ApplnTypeSelect of PurchHeader.FieldNo("Applies-to Doc. No."): ApplnType:=ApplnType::"Applies-to Doc. No.";
        PurchHeader.FieldNo("Applies-to ID"): ApplnType:=ApplnType::"Applies-to ID";
        end;
        SetApplyingVendLedgEntry();
    end;
    procedure SetVendLedgEntry(NewVendLedgEntry: Record "Global Vendor Ledger Entry")
    begin
    ...
    begin
        ...
        if not IsHandled then begin
            case CalcType of CalcType::"Purchase Header": SetApplyingVendLedgEntryPurchaseHeader();
            CalcType::"Gen. Jnl. Line": SetApplyingVendLedgEntryGenJnlLine();
            CalcType::Direct: SetApplyingVendLedgEntryDirect();
            end;
            CalcApplnAmount();
        end;
    ...
    begin
        ...
        if PurchHeader."Document Type" = PurchHeader."Document Type"::"Return Order" then TempApplyingVendLedgEntry."Document Type":=TempApplyingVendLedgEntry."Document Type"::"Credit Memo"
        else
    ...
    begin
        if Rec."Applying Entry" then begin
            if TempApplyingVendLedgEntry."Entry No." <> 0 then VendLedgEntry:=TempApplyingVendLedgEntry;
            CODEUNIT.Run(CODEUNIT::"Global Vend. Entry-Edit", Rec);
    ...
    begin
        ...
        if CurrentRec then VendLedgEntry.SetRecFilter()
        else
            CurrPage.SetSelectionFilter(VendLedgEntry);
    ...
    begin
        ...
        if GenJnlLineApply then VendEntrySetApplID.SetApplId(VendLedgEntry, TempApplyingVendLedgEntry, GenJnlLine."Applies-to ID", GenJnlLine."Bank Document No.")
        else
            VendEntrySetApplID.SetApplId(VendLedgEntry, TempApplyingVendLedgEntry, PurchHeader."Applies-to ID", '');
    end;
    procedure CheckVendLedgEntry(var VendorLedgerEntry: Record "Global Vendor Ledger Entry")
    var
        RaiseError: Boolean;
    begin
        if VendorLedgerEntry.FindSet()then repeat if CalcType = CalcType::"Gen. Jnl. Line" then begin
                    RaiseError:=TempApplyingVendLedgEntry."Posting Date" < VendorLedgerEntry."Posting Date";
                    OnBeforeEarlierPostingDateError(TempApplyingVendLedgEntry, VendorLedgerEntry, RaiseError, CalcType.AsInteger(), PmtDiscAmount);
                    if RaiseError then Error(EarlierPostingDateErr, TempApplyingVendLedgEntry."Document Type", TempApplyingVendLedgEntry."Document No.", VendorLedgerEntry."Document Type", VendorLedgerEntry."Document No.");
                ...
                if TempApplyingVendLedgEntry."Entry No." <> 0 then begin
                    ...
                    GenJnlApply.CheckAgainstApplnCurrency(ApplnCurrencyCode, VendorLedgerEntry."Currency Code", GenJnlLine."Account Type"::Vendor, true);
                end;
    ...
    begin
        ...
        if not IsHandled then begin
            ...
            case CalcType of CalcType::Direct: begin
                FindAmountRounding();
                VendEntryApplID:=CopyStr(UserId(), 1, MaxStrLen(VendEntryApplID));
                if VendEntryApplID = '' then VendEntryApplID:='***';
                ...
                if AppliesToID = '' then AppliedVendLedgEntry.SetRange("Applies-to ID", VendEntryApplID)
                else
                ...
                HandleChosenEntries(CalcType::Direct, VendLedgEntry."Remaining Amount", VendLedgEntry."Currency Code", VendLedgEntry."Posting Date");
            end;
            CalcType::"Gen. Jnl. Line": begin
                FindAmountRounding();
                ...
                case ApplnType of ApplnType::"Applies-to Doc. No.": begin
                    ...
                    if AppliedVendLedgEntry."Amount to Apply" <> 0 then AppliedAmount:=Round(AppliedVendLedgEntry."Amount to Apply", AmountRoundingPrecision)
                    else
                ...
                ApplnType::"Applies-to ID": begin
                    ...
                    HandleChosenEntries(CalcType::"Gen. Jnl. Line", GenJnlLine2.Amount, GenJnlLine2."Currency Code", GenJnlLine2."Posting Date");
                end;
                end;
            end;
            CalcType::"Purchase Header": begin
                FindAmountRounding();
                case ApplnType of ApplnType::"Applies-to Doc. No.": begin
                    AppliedVendLedgEntry:=Rec;
                    AppliedVendLedgEntry.CalcFields("Remaining Amount");
    ...
    protected procedure CalcApplnRemainingAmount(Amount: Decimal): Decimal var
    ...
    procedure CalcApplnAmountToApply(AmountToApply: Decimal): Decimal var
    ...
    begin
        ApplnRounding:=0;
        case CalcType of CalcType::"Purchase Header": exit;
        CalcType::"Gen. Jnl. Line": if(GenJnlLine."Document Type" <> GenJnlLine."Document Type"::Payment) and (GenJnlLine."Document Type" <> GenJnlLine."Document Type"::Refund)then exit;
        end;
        if ApplnCurrencyCode = '' then ApplnRoundingPrecision:=GLSetup."Appln. Rounding Precision"
        else
        begin
    ...
    procedure GetVendLedgEntry(var VendLedgEntry: Record "Global Vendor Ledger Entry")
    begin
        VendLedgEntry:=Rec;
    end;
    ...
    begin
        if CalcType = CalcType::Direct then begin
            ...
            if AppliesToID = '' then VendLedgEntry.SetRange("Applies-to ID", VendEntryApplID)
            else
    ...
    var
        TempAppliedVendLedgEntry: Record "Global Vendor Ledger Entry" temporary;
    ...
    begin
        ...
        if IsHandled then exit;
        if not AppliedVendLedgEntry.FindSet(false)then exit;
        repeat TempAppliedVendLedgEntry:=AppliedVendLedgEntry;
        ...
        repeat if not FromZeroGenJnl then TempAppliedVendLedgEntry.SetRange(Positive, CurrentAmount < 0);
            if TempAppliedVendLedgEntry.FindFirst()then begin
                ...
                //         CanUseDisc := false;
                // end;
                if CanUseDisc and (Abs(TempAppliedVendLedgEntry."Amount to Apply") >= Abs(TempAppliedVendLedgEntry."Remaining Amount" - TempAppliedVendLedgEntry."Remaining Pmt. Disc. Possible"))then if Abs(CurrentAmount) > Abs(TempAppliedVendLedgEntry."Remaining Amount" - TempAppliedVendLedgEntry."Remaining Pmt. Disc. Possible")then begin
            ...
            if OldPmtDisc <> PmtDiscAmount then AppliedAmount+=TempAppliedVendLedgEntry."Remaining Amount"
            else
                AppliedAmount+=TempAppliedVendLedgEntry."Amount to Apply";
            OldPmtDisc:=PmtDiscAmount;
            if PossiblePmtDisc <> 0 then CorrectionAmount:=TempAppliedVendLedgEntry."Remaining Amount" - TempAppliedVendLedgEntry."Amount to Apply"
            else
                CorrectionAmount:=0;
    ...
    var
        RecBeforeRunPostApplicationVendorLedgerEntry: Record "Global Vendor Ledger Entry";
    ...
    begin
        ...
        if CalcType = CalcType::Direct then begin
            if TempApplyingVendLedgEntry."Entry No." <> 0 then begin
                ...
                CopyFromVendLedgEntry(ApplyUnapplyParameters, Rec);
                ...
                if ACTION::OK = PostApplication.RunModal()then begin
                    if Rec."Entry No." <> RecBeforeRunPostApplicationVendorLedgerEntry."Entry No." then Rec:=RecBeforeRunPostApplicationVendorLedgerEntry;
                    ...
                    if not IsHandled then if NewApplyUnapplyParameters."Posting Date" < ApplicationDate then Error(ApplicationDateErr, Rec.FieldCaption("Posting Date"), Rec.TableCaption());
                end
                else
                    exit;
    ...
    //TEC.VJ
    procedure CopyFromVendLedgEntry(VAR ApplyUnapplyParameters: Record "Apply Unapply Parameters"; VendLedgEntry: Record "Global Vendor Ledger Entry")
    begin
        //WITH ApplyUnapplyParameters DO begin
        ApplyUnapplyParameters."Entry No.":=VendLedgEntry."Entry No.";
        ApplyUnapplyParameters."Account Type":=ApplyUnapplyParameters."Account Type"::Vendor;
        ApplyUnapplyParameters."Account No.":=VendLedgEntry."Vendor No.";
        ApplyUnapplyParameters."Posting Date":=VendLedgEntry."Posting Date";
        ApplyUnapplyParameters."Document Type":=VendLedgEntry."Document Type";
        ApplyUnapplyParameters."Document No.":=VendLedgEntry."Document No.";
    //end;
    end;
    procedure GetApplicationDate(VendLedgEntry: Record "Global Vendor Ledger Entry")ApplicationDate: Date var
        ApplyToVendLedgEntry: Record "Global Vendor Ledger Entry";
        IsHandled: Boolean;
    begin
        IsHandled:=false;
        if IsHandled then exit(ApplicationDate);
        ApplicationDate:=0D;
        ...
        repeat if ApplyToVendLedgEntry."Posting Date" > ApplicationDate then ApplicationDate:=ApplyToVendLedgEntry."Posting Date";
        until ApplyToVendLedgEntry.Next() = 0;
    end;
    //TEC.VJ COMMENTED<<
    local procedure CheckActionPerformed()Result: Boolean var
        IsHandled: Boolean;
        ActionPerformed: Boolean;
    begin
        ActionPerformed:=false;
        IsHandled:=false;
        OnBeforeCheckActionPerformed(ActionPerformed, OK, CalcType.AsInteger(), PostingDone, TempApplyingVendLedgEntry, ApplnType.AsInteger(), Result, IsHandled, AppliesToID);
    ...
    protected procedure ExchangeLedgerEntryAmounts(Type: Enum "Vendor Apply Calculation Type"; CurrencyCode: Code[10]; var CalcVendLedgEntry: Record "Global Vendor Ledger Entry"; PostingDate: Date)
    var
        CalculateCurrency: Boolean;
    begin
        CalcVendLedgEntry.CalcFields("Remaining Amount");
        if Type = Type::Direct then CalculateCurrency:=TempApplyingVendLedgEntry."Entry No." <> 0
        else
    ...
    var
        ApplyToVendorLedgerEntry: Record "Global Vendor Ledger Entry";
    ...
    begin
        ...
        if ApplyToVendorLedgerEntry.FindSet()then repeat if not IsFirst then ThereAreEntriesToApply:=(IsPositiv <> ApplyToVendorLedgerEntry.Positive)
                else
                    IsPositiv:=ApplyToVendorLedgerEntry.Positive;
                IsFirst:=false;
                Counter+=1;
            until(ApplyToVendorLedgerEntry.next() = 0) or ThereAreEntriesToApply;
        if not ThereAreEntriesToApply and (Counter > 1)then error(AllEntriesHaveTheSameSignErr)end;
    local procedure ActivateFields()
    begin
        CalledFromEntry:=CalcType = CalcType::Direct;
        AppliesToIDVisible:=ApplnType <> ApplnType::"Applies-to Doc. No.";
    end;
    procedure CalcOppositeEntriesAmount(var TempAppliedVendorLedgerEntry: Record "Global Vendor Ledger Entry" temporary)Result: Decimal var
        SavedAppliedVendorLedgerEntry: Record "Global Vendor Ledger Entry";
        CurrPosFilter: Text;
    begin
        ...
        if CurrPosFilter <> '' then begin
            ...
            if TempAppliedVendorLedgerEntry.FindSet()then repeat TempAppliedVendorLedgerEntry.CalcFields("Remaining Amount");
    ...
    procedure GetApplnCurrencyCode(): Code[10]begin
        exit(ApplnCurrencyCode);
    end;
    procedure GetCalcType(): Integer begin
    ...
    local procedure OnAfterCalcApplnAmount(VendorLedgerEntry: Record "Global Vendor Ledger Entry"; var AppliedAmount: Decimal; var ApplyingAmount: Decimal; CalcType: Enum "Vendor Apply Calculation Type"; var AppliedVendLedgEntry: Record "Global Vendor Ledger Entry"; var GenJournalLine: Record "Gen. Journal Line")
    begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnAfterCalcApplnAmountToApply(VendorLedgerEntry: Record "Global Vendor Ledger Entry"; var ApplnAmountToApply: Decimal; var ApplnAmtToApply: Decimal)
    begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnAfterCalcApplnRemainingAmount(VendorLedgerEntry: Record "Global Vendor Ledger Entry"; var ApplnRemainingAmount: Decimal)
    begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnAfterExchangeLedgerEntryAmounts(var CalcVendorLedgerEntry: Record "Global Vendor Ledger Entry"; VendorLedgerEntry: Record "Global Vendor Ledger Entry"; CurrencyCode: Code[10])
    begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnAfterOpenPage(var VendorLedgerEntry: Record "Global Vendor Ledger Entry"; var ApplyingVendLedgEntry: Record "Global Vendor Ledger Entry")
    begin
    end;
    [IntegrationEvent(true, false)]
    local procedure OnAfterSetApplyingVendLedgEntry(var TempApplyingVendorLedgerEntry: Record "Global Vendor Ledger Entry"; GenJournalLine: Record "Gen. Journal Line"; PurchaseHeader: Record "Purchase Header"; CalcType: Enum "Vendor Apply Calculation Type")
    begin
    end;
    [IntegrationEvent(true, false)]
    local procedure OnBeforeCallVendEntrySetApplIDSetApplId(VendEntrySetApplID: Codeunit "Global Vend. Entry-SetAppl.ID"; var VendorLedgerEntry: Record "Global Vendor Ledger Entry"; var TempApplyingVendLedgEntry: Record "Global Vendor Ledger Entry"; var IsHandled: Boolean)
    begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCalcApplnAmount(var VendorLedgerEntry: Record "Global Vendor Ledger Entry"; var GenJournalLine: Record "Gen. Journal Line"; PurchaseHeader: record "Purchase Header"; AppliedVendLedgEntry: Record "Global Vendor Ledger Entry"; CalcType: Enum "Vendor Apply Calculation Type"; ApplnType: Enum "Vendor Apply-to Type"; var IsHandled: Boolean)
    begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCheckActionPerformed(ActionPerformed: Boolean; OK: Boolean; CalcType: Option Direct, GenJnlLine, PurchHeader; PostingDone: Boolean; ApplyingVendLedgEntry: Record "Global Vendor Ledger Entry" temporary; ApplnType: Option " ", "Applies-to Doc. No.", "Applies-to ID"; var Result: Boolean; var IsHandled: Boolean; AppliesToID: Code[50])
    begin
    end;
    [IntegrationEvent(true, false)]
    local procedure OnFindApplyingEntryOnBeforeCalcApplnAmount(var VendorLedgerEntry: Record "Global Vendor Ledger Entry")
    begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnFindApplyingEntryOnBeforeAssignTempApplyingVendLedgEntry(var VendorLedgerEntry: Record "Global Vendor Ledger Entry")
    begin
    end;
    [IntegrationEvent(true, false)]
    local procedure OnBeforeHandledChosenEntries(Type: Option Direct, GenJnlLine, PurchHeader; CurrentAmount: Decimal; CurrencyCode: Code[10]; PostingDate: Date; var AppliedVendLedgEntry: Record "Global Vendor Ledger Entry"; var IsHandled: Boolean; var VendorLedgerEntry: Record "Global Vendor Ledger Entry")
    begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnBeforeEarlierPostingDateError(ApplyingVendLedgEntry: Record "Global Vendor Ledger Entry"; VendorLedgerEntry: Record "Global Vendor Ledger Entry"; var RaiseError: Boolean; CalcType: Option; PmtDiscAmount: Decimal)
    begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnBeforePostDirectApplication(var VendorLedgerEntry: Record "Global Vendor Ledger Entry"; PreviewMode: Boolean; var IsHandled: Boolean)
    begin
    end;
    [IntegrationEvent(true, false)]
    local procedure OnBeforeSetApplyingVendLedgEntry(var ApplyingVendLedgEntry: Record "Global Vendor Ledger Entry"; GenJournalLine: Record "Gen. Journal Line"; PurchHeader: Record "Purchase Header"; var CalcType: enum "Vendor Apply Calculation Type"; var IsHandled: Boolean)
    begin
    end;
    [IntegrationEvent(true, false)]
    local procedure OnBeforeRunVendEntryEdit(var VendorLedgerEntry: Record "Global Vendor Ledger Entry")
    begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnFindApplyingEntryOnAfterSetFilters(ApplyingVendLedgEntry: Record "Global Vendor Ledger Entry"; var VendorLedgerEntry: Record "Global Vendor Ledger Entry")
    begin
    end;
    [IntegrationEvent(true, false)]
    local procedure OnHandleChosenEntriesOnAfterTempAppliedVendLedgEntryInsert(var TempAppliedVendorLedgerEntry: Record "Global Vendor Ledger Entry" temporary)
    begin
    end;
    [IntegrationEvent(false, false)]
    ...
    [IntegrationEvent(true, false)]
    local procedure OnSetApplyingVendLedgEntryOnBeforeCalcTypeDirectCalcApplnAmount(var ApplyingAmount: Decimal; var ApplyingVendorLedgerEntry: Record "Global Vendor Ledger Entry"; var VendorLedgerEntry: Record "Global Vendor Ledger Entry")
    begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnSetVendApplIdOnAfterCheckAgainstApplnCurrency(var VendorLedgerEntry: Record "Global Vendor Ledger Entry"; CalcType: Option Direct, GenJnlLine, PurchHeader; GenJnlLine: Record "Gen. Journal Line"; PurchHeader: Record "Purchase Header"; ApplyingVendLedgEntry: Record "Global Vendor Ledger Entry")
    begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnAfterSetApplyingVendLedgEntryPurchaseHeader(var TempApplyingVendLedgEntry: Record "Global Vendor Ledger Entry" temporary; var PurchHeader: Record "Purchase Header")
    begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnAfterSetApplyingVendLedgEntryGenJnlLine(var TempApplyingVendLedgEntry: Record "Global Vendor Ledger Entry" temporary; var GenJnlLine: Record "Gen. Journal Line")
    begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnCheckVendLedgEntryOnBeforeCheckAgainstApplnCurrency(var GenJournalLine: Record "Gen. Journal Line"; VendorLedgerEntry: Record "Global Vendor Ledger Entry")
    begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnCheckVendLedgEntryOnAfterEarlierPostingDateTest(ApplyingVendorLedgerEntry: Record "Global Vendor Ledger Entry"; VendorLedgerEntry: Record "Global Vendor Ledger Entry"; CalcType: Enum "Vendor Apply Calculation Type"; var OK: Boolean)
    begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnQueryClosePageOnAfterEarlierPostingDateTest(ApplyingVendorLedgerEntry: Record "Global Vendor Ledger Entry"; VendorLedgerEntry: Record "Global Vendor Ledger Entry"; CalcType: Enum "Vendor Apply Calculation Type"; var OK: Boolean)
    begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnCalcApplnAmountOnAfterAppliedVendLedgEntrySetFilter(var AppliedVendorLedgerEntry: Record "Global Vendor Ledger Entry"; VendorLedgerEntry: Record "Global Vendor Ledger Entry"; var RecVendorLedgerEntry: Record "Global Vendor Ledger Entry")
    begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnSetVendApplIdOnAfterSetFilter(var RecVendorLedgerEntry: Record "Global Vendor Ledger Entry"; CurrentRec: Boolean; var VendorLedgerEntry: Record "Global Vendor Ledger Entry"; var TempApplyingVendorLedgerEntry: Record "Global Vendor Ledger Entry" temporary)
    begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnModifyRecordOnBeforeRunCodeunitVendEntryEdit(VendorApplyCalculationType: Enum "Vendor Apply Calculation Type"; var VendorLedgerEntry: Record "Global Vendor Ledger Entry"; var IsHandled: Boolean)
    begin
    end;
    [IntegrationEvent(false, false)]
    local procedure OnBeforeOnQueryClosePage(CloseAction: Action; ApplyingVendorLedgerEntry: Record "Global Vendor Ledger Entry"; ApplnType: Enum "Vendor Apply-to Type"; VendorLedgerEntry: Record "Global Vendor Ledger Entry"; CalcType: Enum "Vendor Apply Calculation Type"; var IsHandled: Boolean)
    begin
    end;
    [IntegrationEvent(true, false)]
    local procedure OnPostDirectApplicationOnBeforeCheckApplicationDate(VendorLedgerEntry: Record "Global Vendor Ledger Entry"; NewApplyUnapplyParameters: Record "Apply Unapply Parameters"; ApplicationDate: Date; PreviewMode: Boolean; var IsHandled: Boolean)
    begin
    end;
}
