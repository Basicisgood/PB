report 50157 "Adjust Exchange Rates PB"
{
    ApplicationArea = Basic, Suite;
    Caption = 'Adjust Exch. Rates PB';
    ...
    dataset
    {
        dataitem(Currency; Currency)
        {
            ...
            dataitem("Bank Account"; "Bank Account")
            {
                ...
                DataItemTableView = sorting("Bank Acc. Posting Group")where("Exclude Revaluation"=const(false));
                ...
                dataitem(BankAccountGroupTotal; "Integer")
                {
                    ...
                    begin
                        ...
                        if BankAccount.Next() = 1 then begin
                            if BankAccount."Bank Acc. Posting Group" <> "Bank Account"."Bank Acc. Posting Group" then GroupTotal:=true;
                        end
                        else
                            GroupTotal:=true;
                        if GroupTotal then if TotalAdjAmount <> 0 then begin
                                AdjExchRateBufferUpdate("Bank Account"."Currency Code", "Bank Account"."Bank Acc. Posting Group", TotalAdjBase, TotalAdjBaseLCY, TotalAdjAmount, 0, 0, 0, PostingDate, '');
                ...
                begin
                    ...
                    CurrAdjAmount:=Round(CurrExchRate.ExchangeAmtFCYToLCYAdjmt(PostingDate, Currency.Code, "Balance at Date", Currency."Currency Factor")) - "Balance at Date (LCY)";
                ...
                begin
                    if not AdjBank then CurrReport.Break();
            ...
            begin
                if(Code = '') and (AdjCust or AdjVend or AdjBank or AdjRevalAcc)then //PS001
 Error(Text011Err);
            end;
            trigger OnPreDataItem()
            begin
                CheckPostingDate();
                //if (not AdjCust) and (not AdjVend) and (not AdjBank)and (not AdjRevalAcc) then    //PS001
                if(not AdjCust) and (not AdjVend) and (not AdjBank) and (not AdjRevalAcc)then //PS001
 CurrReport.Break();
                Window.Open(Text006Txt + Text007Txt + Text008Txt + Text009Txt + Text010Txt);
                CustNoTotal:=Customer.Count();
                VendNoTotal:=Vendor.Count();
                RevalAccTotal:="G/L Revaluation Account".Count; //PS001
        ...
        dataitem(Customer; Customer)
        {
            ...
            dataitem(CustomerLedgerEntryLoop; "Integer")
            {
                ...
                begin
                    ...
                    if FirstEntry then begin
                        ...
                        FirstEntry:=false end
                    else if TempCustLedgerEntry.Next() = 0 then CurrReport.Break();
                    CustLedgerEntry.Get(TempCustLedgerEntry."Entry No.");
                    AdjustCustomerLedgerEntry(CustLedgerEntry, PostingDate);
                end;
                trigger OnPreDataItem()
                begin
                    if not TempCustLedgerEntry.Find('-')then CurrReport.Break();
                    FirstEntry:=true;
                end;
            }
            trigger OnAfterGetRecord()
            ...
            trigger OnPostDataItem()
            begin
                if CustNo <> 0 then HandlePostAdjmt(1); // Customer
            end;
            trigger OnPreDataItem()
            begin
                if not AdjCust then CurrReport.Break();
                DtldCustLedgEntry.LockTable();
                CustLedgerEntry.LockTable();
                CustNo:=0;
                if DtldCustLedgEntry.Find('+')then NewEntryNo:=DtldCustLedgEntry."Entry No." + 1
                else
                    NewEntryNo:=1;
                Clear(DimMgt);
            end;
        ...
        dataitem(Vendor; Vendor)
        {
            ...
            dataitem(VendorLedgerEntryLoop; "Integer")
            {
                ...
                begin
                    ...
                    if FirstEntry then begin
                        ...
                        FirstEntry:=false end
                    else if TempVendorLedgerEntry.Next() = 0 then CurrReport.Break();
                    VendorLedgerEntry.Get(TempVendorLedgerEntry."Entry No.");
                    AdjustVendorLedgerEntry(VendorLedgerEntry, PostingDate);
                end;
                trigger OnPreDataItem()
                begin
                    if not TempVendorLedgerEntry.Find('-')then CurrReport.Break();
                    FirstEntry:=true;
                end;
            }
            trigger OnAfterGetRecord()
            ...
            trigger OnPostDataItem()
            begin
                if VendNo <> 0 then HandlePostAdjmt(2); // Vendor
            end;
            trigger OnPreDataItem()
            begin
                if not AdjVend then CurrReport.Break();
                DtldVendLedgEntry.LockTable();
                VendorLedgerEntry.LockTable();
                VendNo:=0;
                if DtldVendLedgEntry.Find('+')then NewEntryNo:=DtldVendLedgEntry."Entry No." + 1
                else
                    NewEntryNo:=1;
                Clear(DimMgt);
            end;
        ...
        dataitem("VAT Posting Setup"; "VAT Posting Setup")
        {
            ...
            begin
                ...
                if "VAT Calculation Type" <> "VAT Calculation Type"::"Sales Tax" then begin
                    ...
                    if(VATEntry2.Amount <> 0) or (VATEntry2."Additional-Currency Amount" <> 0)then begin
                        AdjustVATAccount(GetPurchAccount(false), VATEntry2.Amount, VATEntry2."Additional-Currency Amount", VATEntryTotalBase.Amount, VATEntryTotalBase."Additional-Currency Amount");
                        if "VAT Calculation Type" = "VAT Calculation Type"::"Reverse Charge VAT" then AdjustVATAccount(GetRevChargeAccount(false), -VATEntry2.Amount, -VATEntry2."Additional-Currency Amount", -VATEntryTotalBase.Amount, -VATEntryTotalBase."Additional-Currency Amount");
                    end;
                    if(VATEntry2."Remaining Unrealized Amount" <> 0) or (VATEntry2."Add.-Curr. Rem. Unreal. Amount" <> 0)then begin
                        TestField("Unrealized VAT Type");
                        AdjustVATAccount(GetPurchAccount(true), VATEntry2."Remaining Unrealized Amount", VATEntry2."Add.-Curr. Rem. Unreal. Amount", VATEntryTotalBase."Remaining Unrealized Amount", VATEntryTotalBase."Add.-Curr. Rem. Unreal. Amount");
                        if "VAT Calculation Type" = "VAT Calculation Type"::"Reverse Charge VAT" then AdjustVATAccount(GetRevChargeAccount(true), -VATEntry2."Remaining Unrealized Amount", -VATEntry2."Add.-Curr. Rem. Unreal. Amount", -VATEntryTotalBase."Remaining Unrealized Amount", -VATEntryTotalBase."Add.-Curr. Rem. Unreal. Amount");
                    end;
                    AdjustVATEntries(VATEntry.Type::Sale, false);
                    if(VATEntry2.Amount <> 0) or (VATEntry2."Additional-Currency Amount" <> 0)then AdjustVATAccount(GetSalesAccount(false), VATEntry2.Amount, VATEntry2."Additional-Currency Amount", VATEntryTotalBase.Amount, VATEntryTotalBase."Additional-Currency Amount");
                    if(VATEntry2."Remaining Unrealized Amount" <> 0) or (VATEntry2."Add.-Curr. Rem. Unreal. Amount" <> 0)then begin
                        TestField("Unrealized VAT Type");
                        AdjustVATAccount(GetSalesAccount(true), VATEntry2."Remaining Unrealized Amount", VATEntry2."Add.-Curr. Rem. Unreal. Amount", VATEntryTotalBase."Remaining Unrealized Amount", VATEntryTotalBase."Add.-Curr. Rem. Unreal. Amount");
                    end;
                end
                else
                begin
                    if TaxJurisdiction.Find('-')then repeat VATEntry.SetRange("Tax Jurisdiction Code", TaxJurisdiction.Code);
                            AdjustVATEntries(VATEntry.Type::Purchase, false);
            ...
            trigger OnPreDataItem()
            begin
                if not AdjGLAcc or (GLSetup."VAT Exchange Rate Adjustment" = GLSetup."VAT Exchange Rate Adjustment"::"No Adjustment")then CurrReport.Break();
                VATEntryNoTotal:=VATEntry.Count();
                if VATEntryNoTotal = 0 then CurrReport.Break();
                Window.Open(Text012Txt + Text013Txt);
                if GLSetup."Journal Templ. Name Mandatory" then VATEntry.SetCurrentKey("Journal Templ. Name", Type, Closed, "VAT Bus. Posting Group", "VAT Prod. Posting Group", "Document Type", "Posting Date")
                else if not VATEntry.SetCurrentKey(Type, Closed, "VAT Bus. Posting Group", "VAT Prod. Posting Group", "Posting Date")then VATEntry.SetCurrentKey(Type, Closed, "Tax Jurisdiction Code", "Use Tax", "Posting Date");
        ...
        dataitem("G/L Account"; "G/L Account")
        {
            ...
            begin
                ...
                if "Exchange Rate Adjustment" = "Exchange Rate Adjustment"::"No Adjustment" then CurrReport.Skip();
                TempDimSetEntry.Reset();
                TempDimSetEntry.DeleteAll();
                CalcFields("Net Change", "Additional-Currency Net Change");
                case "Exchange Rate Adjustment" of "Exchange Rate Adjustment"::"Adjust Amount": PostGLAccAdjmt("No.", "Exchange Rate Adjustment"::"Adjust Amount", Round(CurrExchRate2.ExchangeAmtFCYToLCYAdjmt(PostingDate, GLSetup."Additional Reporting Currency", "Additional-Currency Net Change", AddCurrCurrencyFactor) - "Net Change"), "Net Change", "Additional-Currency Net Change");
                "Exchange Rate Adjustment"::"Adjust Additional-Currency Amount": PostGLAccAdjmt("No.", "Exchange Rate Adjustment"::"Adjust Additional-Currency Amount", Round(CurrExchRate2.ExchangeAmtLCYToFCY(PostingDate, GLSetup."Additional Reporting Currency", "Net Change", AddCurrCurrencyFactor) - "Additional-Currency Net Change", Currency3."Amount Rounding Precision"), "Net Change", "Additional-Currency Net Change");
                end;
            end;
            trigger OnPostDataItem()
            begin
            ...
            trigger OnPreDataItem()
            begin
                if not AdjGLAcc then CurrReport.Break();
                Window.Open(Text014Txt + Text015Txt);
                GLAccNoTotal:=Count;
                SetRange("Date Filter", StartDate, EndDate);
            //TEC-SGarg - Added>>- no need
            //IF G_GLAccNoFilter <> '' then begin
            //    SetRange("No.", G_GLAccNoFilter);
            //end;
            //TEC-SGarg - Added<<- no ned
            end;
        }
        //PS001 Start
        dataitem("G/L Revaluation Account"; "G/L Account")
        {
            //RequestFilterFields = "No.";
            DataItemTableView = sorting("No.")where("Include Revaluation"=const(true));

            dataitem(RevaluationGLEntryLoop; "Integer")
            {
                DataItemTableView = sorting(Number);

                dataitem("Detailed G/L Entry"; "Detailed G/L Entry PB")
                {
                    DataItemTableView = sorting("G/L Entry No.", "Posting Date");

                    trigger OnAfterGetRecord()
                    begin
                        AdjustgLEntry(gLEntry, "Posting Date");
                    end;
                    trigger OnPreDataItem()
                    begin
                ...
                begin
                    //TempDtldCustLedgEntrySums.DeleteAll();
                    TempDtldGLEntrySums.DeleteAll();
                    if FirstEntry then begin
                        TempGLEntry.Find('-');
                        FirstEntry:=false end
                    else if TempGLEntry.Next() = 0 then CurrReport.Break();
                    GLEntry.Get(TempGLEntry."Entry No.");
                    AdjustgLEntry(gLEntry, GLEntry."Posting Date");
                end;
                trigger OnPreDataItem()
                begin
                    if not TempGLEntry.Find('-')then CurrReport.Break();
                    FirstEntry:=true;
                end;
            }
            trigger OnAfterGetRecord()
            begin
                RevalGlAccNo:=RevalGlAccNo + 1;
                Window.Update(2, Round(RevalGlAccNo / RevalAccTotal * 10000, 1));
                PrepareTempgLEntry("G/L Revaluation Account", TempglEntry);
            //OnCustomerAfterGetRecordOnAfterFindCustLedgerEntriesToAdjust(TempCustLedgerEntry);
            end;
            trigger OnPostDataItem()
            begin
                if RevalGlAccNo <> 0 then HandlePostAdjmt(3); // Revaluation G/L Account
            end;
            trigger OnPreDataItem()
            begin
                if not AdjRevalAcc then CurrReport.Break();
                DtldgLEntry.LockTable();
                gLEntry.LockTable();
                if RevalAccount <> '' then SetRange("No.", RevalAccount);
                RevalGlAccNo:=0;
                if DtldgLEntry.Find('+')then NewEntryNo:=DtldgLentry."Entry No." + 1
                else
                    NewEntryNo:=1;
                Clear(DimMgt);
            end;
    ...
    requestpage
    {
                ...
                group(Options)
                {
                    ...
                    field(JournalBatchName; GenJnlLineReq."Journal Batch Name")
                    {
                        ...
                        begin
                            ...
                            if GenJnlLineReq."Journal Batch Name" <> '' then GenJnlBatch.Get(GenJnlLineReq."Journal Template Name", GenJnlLineReq."Journal Batch Name");
                        end;
                        trigger OnValidate()
                        begin
                    ...
                    //PS001 Start
                    field(AdjRevalAcc; AdjRevalAcc)
                    {
                        ApplicationArea = Suite;
                        Caption = 'Adjust G/L Revaluation Account';
                    }
                    field(RevalAccount; RevalAccount)
                    {
                        Caption = 'G/L Revaluation Account';
                        TableRelation = "G/L Account" where("Include Revaluation"=const(true));
                        ApplicationArea = All;
                    }
                }
            }
        ...
        trigger OnOpenPage()
        begin
            if PostingDescription = '' then PostingDescription:=Text016Txt;
    ...
    begin
        if GenJnlPostLine.IsGLEntryInconsistent()then GenJnlPostLine.ShowInconsistentEntries()
        else
        begin
            UpdateAnalysisView.UpdateAll(0, true);
            if TotalCustomersAdjusted + TotalVendorsAdjusted + TotalBankAccountsAdjusted + TotalGLAccountsAdjusted + TotalRevalAccAdjusted < 1 then Message(NothingToAdjustMsg)
            else
    ...
    trigger OnPreReport()
    begin
        if EndDateReq = 0D then EndDate:=DMY2Date(31, 12, 9999)
        else
            EndDate:=EndDateReq;
        GLSetup.Get();
        if 1 = 1 then begin
            if GenJnlLineReq."Journal Template Name" = '' then Error(PleaseEnterErr, GenJnlLineReq.FieldCaption("Journal Template Name"));
            if GenJnlLineReq."Journal Batch Name" = '' then Error(PleaseEnterErr, GenJnlLineReq.FieldCaption("Journal Batch Name"));
            Clear(NoSeries);
            Clear(PostingDocNo);
            GenJnlBatch.Get(GenJnlLineReq."Journal Template Name", GenJnlLineReq."Journal Batch Name");
            GenJnlBatch.TestField("No. Series");
            PostingDocNo:=NoSeries.GetNextNo(GenJnlBatch."No. Series", PostingDate, true);
        end
        else if PostingDocNo = '' then Error(Text000Err);
        if(not AdjCust) and (not AdjVend) and (not AdjBank) and AdjGLAcc then if not Confirm(Text001Txt + Text004Txt, false)then Error(Text005Err);
        SourceCodeSetup.Get();
        if ExchRateAdjReg.FindLast()then ExchRateAdjReg.Init();
        if AdjGLAcc then begin
            ...
            if VATPostingSetup2.Find('-')then repeat if VATPostingSetup2."VAT Calculation Type" <> VATPostingSetup2."VAT Calculation Type"::"Sales Tax" then begin
                        CheckExchRateAdjustment(VATPostingSetup2."Purchase VAT Account", VATPostingSetup2.TableCaption(), VATPostingSetup2.FieldCaption("Purchase VAT Account"));
                        CheckExchRateAdjustment(VATPostingSetup2."Reverse Chrg. VAT Acc.", VATPostingSetup2.TableCaption(), VATPostingSetup2.FieldCaption("Reverse Chrg. VAT Acc."));
                        CheckExchRateAdjustment(VATPostingSetup2."Purch. VAT Unreal. Account", VATPostingSetup2.TableCaption(), VATPostingSetup2.FieldCaption("Purch. VAT Unreal. Account"));
                        CheckExchRateAdjustment(VATPostingSetup2."Reverse Chrg. VAT Unreal. Acc.", VATPostingSetup2.TableCaption(), VATPostingSetup2.FieldCaption("Reverse Chrg. VAT Unreal. Acc."));
                        CheckExchRateAdjustment(VATPostingSetup2."Sales VAT Account", VATPostingSetup2.TableCaption(), VATPostingSetup2.FieldCaption("Sales VAT Account"));
                        CheckExchRateAdjustment(VATPostingSetup2."Sales VAT Unreal. Account", VATPostingSetup2.TableCaption(), VATPostingSetup2.FieldCaption("Sales VAT Unreal. Account"));
                    end;
                until VATPostingSetup2.Next() = 0;
            //with TaxJurisdiction2 do
            if TaxJurisdiction2.Find('-')then repeat CheckExchRateAdjustment(TaxJurisdiction2."Tax Account (Purchases)", TaxJurisdiction2.TableCaption(), TaxJurisdiction2.FieldCaption("Tax Account (Purchases)"));
                    CheckExchRateAdjustment(TaxJurisdiction2."Reverse Charge (Purchases)", TaxJurisdiction2.TableCaption(), TaxJurisdiction2.FieldCaption("Reverse Charge (Purchases)"));
                    CheckExchRateAdjustment(TaxJurisdiction2."Unreal. Tax Acc. (Purchases)", TaxJurisdiction2.TableCaption(), TaxJurisdiction2.FieldCaption("Unreal. Tax Acc. (Purchases)"));
                    CheckExchRateAdjustment(TaxJurisdiction2."Unreal. Rev. Charge (Purch.)", TaxJurisdiction2.TableCaption(), TaxJurisdiction2.FieldCaption("Unreal. Rev. Charge (Purch.)"));
                    CheckExchRateAdjustment(TaxJurisdiction2."Tax Account (Sales)", TaxJurisdiction2.TableCaption(), TaxJurisdiction2.FieldCaption("Tax Account (Sales)"));
                    CheckExchRateAdjustment(TaxJurisdiction2."Unreal. Tax Acc. (Sales)", TaxJurisdiction2.TableCaption(), TaxJurisdiction2.FieldCaption("Unreal. Tax Acc. (Sales)"));
                until TaxJurisdiction2.Next() = 0;
            AddCurrCurrencyFactor:=CurrExchRate2.ExchangeRateAdjmt(PostingDate, GLSetup."Additional Reporting Currency");
        end;
    end;
    var DtldCustLedgEntry: Record "Detailed Cust. Ledg. Entry";
    RevalAccount: Code[20];
    DtldGLEntry: Record "Detailed G/L Entry PB";
    TempDtldCustLedgEntry: Record "Detailed Cust. Ledg. Entry" temporary;
    TempDtldgLEntry: Record "Detailed G/L Entry PB" temporary;
    TempDtldCustLedgEntrySums: Record "Detailed Cust. Ledg. Entry" temporary;
    TempDtldGLEntrySums: Record "Detailed G/L Entry PB" temporary;
    ...
    CustNo: Decimal;
    RevalGlAccNo: Decimal;
    CustNoTotal: Decimal;
    RevalAccTotal: Decimal;
    VendNo: Decimal;
    VendNoTotal: Decimal;
    BankAccNo: Decimal;
    ...
    local procedure PostAdjmt(GLAccNo: Code[20]; PostingAmount: Decimal; AdjBase2: Decimal; CurrencyCode2: Code[10]; var DimSetEntry: Record "Dimension Set Entry"; PostingDate2: Date; ICCode: Code[20])TransactionNo: Integer var
        GenJnlLine: Record "Gen. Journal Line";
    begin
        if PostingAmount = 0 then exit;
        GenJnlLine.Init();
        ...
        if CurrencyCode2 = GLSetup."Additional Reporting Currency" then GenJnlLine."Source Currency Amount":=0;
    ...
    local procedure PostAdjmtNew(GLAccNo: Code[20]; PostingAmount: Decimal; AdjBase2: Decimal; CurrencyCode2: Code[10]; var DimSetEntry: Record "Dimension Set Entry"; PostingDate2: Date; ICCode: Code[20])TransactionNo: Integer var //New fxn copy from standard
        GenJnlLine: Record "Gen. Journal Line";
    begin
        if PostingAmount = 0 then exit;
        GenJnlLine.Init();
        ...
        GenJnlLine."Journal Batch Name":=GenJnlLineReq."Journal Batch Name";
        GenJnlLine."System-Created Entry":=true;
        GenJnlLine."GL Reval":=true; // Sgarg- Added - to identify reval entries
        TransactionNo:=PostGenJnlLine(GenJnlLine, DimSetEntry);
        OnAfterPostAdjmt(GenJnlLine);
    end;
    ...
    begin
        ...
        if Currency.Code = GLSetup."Additional Reporting Currency" then GenJnlLine."Source Currency Amount":=0;
        GenJnlLine."Source Code":=SourceCodeSetup."Exchange Rate Adjmt.";
        GenJnlLine."System-Created Entry":=true;
        GenJnlLine."Journal Template Name":=GenJnlLineReq."Journal Template Name";
        GenJnlLine."Journal Batch Name":=GenJnlLineReq."Journal Batch Name";
        GenJnlLine.Validate("Shortcut Dimension 2 Code", GenJnlLine."Journal Template Name");
        ...
        if CurrAdjAmount <> 0 then begin
            ...
            if CurrAdjAmount > 0 then AccNo:=GetRealizedGainsAccount(Currency)
            else
                AccNo:=GetRealizedLossesAccount(Currency);
            PostAdjmt(AccNo, -CurrAdjAmount, CurrAdjBase, BankAccount."Currency Code", TempDimSetEntry, PostingDate, '');
        end;
    end;
    local procedure GetRealizedGainsAccount(Currency: Record Currency)AccountNo: Code[20]var
        IsHandled: Boolean;
    begin
        IsHandled:=false;
        OnBeforeGetRealizedGainsAccount(Currency, AccountNo, IsHandled);
        if IsHandled then exit(AccountNo);
        exit(Currency.GetRealizedGainsAccount());
    end;
    local procedure GetRealizedLossesAccount(Currency: Record Currency)AccountNo: Code[20]var
        IsHandled: Boolean;
    begin
        IsHandled:=false;
    ...
    begin
        ...
        TempDtldCVLedgEntryBuf."Transaction No.":=PostAdjmt(CustPostingGr.GetReceivablesAccount(), AdjExchRateBuffer.AdjAmount, AdjExchRateBuffer.AdjBase, AdjExchRateBuffer."Currency Code", TempDimSetEntry, AdjExchRateBuffer."Posting Date", AdjExchRateBuffer."IC Partner Code");
    ...
    begin
        ...
        TempDtldCVLedgEntryBuf."Transaction No.":=PostAdjmt(VendPostingGr.GetPayablesAccount(), AdjExchRateBuffer.AdjAmount, AdjExchRateBuffer.AdjBase, AdjExchRateBuffer."Currency Code", TempDimSetEntry, AdjExchRateBuffer."Posting Date", AdjExchRateBuffer."IC Partner Code");
        if TempDtldCVLedgEntryBuf.Insert()then;
    ...
    begin
        if TempCurrencyToAdjust.Code <> CurrencyCode then TempCurrencyToAdjust.Get(CurrencyCode);
    ...
    begin
        ...
        if EndDate = 0D then EndDateReq:=DMY2Date(31, 12, 9999)
        else
            EndDateReq:=EndDate;
    end;
    ...
    local procedure AdjExchRateBufferUpdate(CurrencyCode2: Code[10]; PostingGroup2: Code[20]; AdjBase2: Decimal; AdjBaseLCY2: Decimal; AdjAmount2: Decimal; GainsAmount2: Decimal; LossesAmount2: Decimal; DimEntryNo: Integer; Postingdate2: Date; ICCode: Code[20]): Integer begin
    ...
    local procedure AdjExchRateBufferUpdateNew(CurrencyCode2: Code[10]; PostingGroup2: Code[20]; AdjBase2: Decimal; AdjBaseLCY2: Decimal; AdjAmount2: Decimal; GainsAmount2: Decimal; LossesAmount2: Decimal; DimEntryNo: Integer; Postingdate2: Date; ICCode: Code[20]; Accno: Code[20]): Integer begin
        TempAdjExchRateBuffer.Init();
        OK:=TempAdjExchRateBuffer.Get(CurrencyCode2, PostingGroup2, DimEntryNo, Postingdate2, ICCode);
        TempAdjExchRateBuffer.AdjBase+=AdjBase2;
        TempAdjExchRateBuffer.AdjBaseLCY+=AdjBaseLCY2;
    ...
    begin
        ...
        if TempAdjExchRateBuffer2.Find('-')then repeat TempAdjExchRateBuffer.SetRange("Currency Code", TempAdjExchRateBuffer2."Currency Code");
                ...
                repeat TempDtldCVLedgEntryBuf.Init();
                    TempDtldCVLedgEntryBuf."Entry No.":=TempAdjExchRateBuffer.Index;
                    if TempAdjExchRateBuffer.AdjAmount <> 0 then case AdjustAccType of 1: // Customer
 ...
 PostVendAdjmt(TempAdjExchRateBuffer, TempDtldCVLedgEntryBuf, TempDimSetEntry);
                        3: //Revaluation G/L Account PS001
 PostGLAdjmt(TempAdjExchRateBuffer, TempDtldCVLedgEntryBuf, TempDimSetEntry);
                        end;
                until TempAdjExchRateBuffer.Next() = 0;
                TempCurrencyToAdjust.Get(TempAdjExchRateBuffer2."Currency Code");
                if TempAdjExchRateBuffer2.TotalGainsAmount <> 0 then PostAdjmt(GetUnrealizedGainsAccount(TempCurrencyToAdjust), -TempAdjExchRateBuffer2.TotalGainsAmount, -TempAdjExchRateBuffer2.AdjBase, TempAdjExchRateBuffer2."Currency Code", TempDimSetEntry, TempAdjExchRateBuffer2."Posting Date", TempAdjExchRateBuffer2."IC Partner Code");
                if TempAdjExchRateBuffer2.TotalLossesAmount <> 0 then PostAdjmt(GetUnrealizedLossesAccount(TempCurrencyToAdjust), -TempAdjExchRateBuffer2.TotalLossesAmount, -TempAdjExchRateBuffer2.AdjBase, TempAdjExchRateBuffer2."Currency Code", TempDimSetEntry, TempAdjExchRateBuffer2."Posting Date", TempAdjExchRateBuffer2."IC Partner Code");
            until TempAdjExchRateBuffer2.Next() = 0;
        case AdjustAccType of 1: // Customer
 ...
 InsertVendLedgEntries(TempDtldVendLedgEntry, TempDtldCVLedgEntryBuf);
        3: // Revaluation G/L Account   PS001
 InsertGLEntries(TempDtldgLEntry, TempDtldCVLedgEntryBuf);
        end;
        ResetTempAdjmtBuffer();
    ...
    begin
        if TempAdjExchRateBuffer.Find('-')then // Summarize per currency and dimension combination
            repeat TempAdjExchRateBuffer2.Init();
                OK:=TempAdjExchRateBuffer2.Get(TempAdjExchRateBuffer."Currency Code", '', TempAdjExchRateBuffer."Dimension Entry No.", TempAdjExchRateBuffer."Posting Date", TempAdjExchRateBuffer."IC Partner Code");
    ...
    begin
        ...
        if TempDtldCustLedgEntry.Find('-')then repeat if TempDtldCVLedgEntryBuf.Get(TempDtldCustLedgEntry."Transaction No.")then TempDtldCustLedgEntry."Transaction No.":=TempDtldCVLedgEntryBuf."Transaction No."
                else
    ...
    begin
        GLEntry.GetLastEntry(LastEntryNo, LastTransactionNo);
        if TempDtldVendLedgEntry.Find('-')then repeat if TempDtldCVLedgEntryBuf.Get(TempDtldVendLedgEntry."Transaction No.")then TempDtldVendLedgEntry."Transaction No.":=TempDtldCVLedgEntryBuf."Transaction No."
                else
    ...
    begin
        ...
        // CustLedgerEntry2.SetRange("Exclude Revaluation", false); //PS001
        ...
        if DtldCustLedgEntry2.Find('-')then repeat CustLedgerEntry2."Entry No.":=DtldCustLedgEntry2."Cust. Ledger Entry No.";
                if CustLedgerEntry2.Find('=')then if(CustLedgerEntry2."Posting Date" >= StartDate) and (CustLedgerEntry2."Posting Date" <= EndDate)then begin
        ...
        //CustLedgerEntry2.SetRange("RIE Exclude Revaluation", false); //PS001
        if CustLedgerEntry2.Find('-')then repeat TempCustLedgerEntry."Entry No.":=CustLedgerEntry2."Entry No.";
                if TempCustLedgerEntry.Insert()then;
    ...
    begin
        ...
        // vendorLedgerEntry2.SetRange("RIE Exclude Revaluation", false); //PS001
        ...
        if DtldVendLedgEntry2.Find('-')then repeat VendorLedgerEntry2."Entry No.":=DtldVendLedgEntry2."Vendor Ledger Entry No.";
                if VendorLedgerEntry2.Find('=')then if(VendorLedgerEntry2."Posting Date" >= StartDate) and (VendorLedgerEntry2."Posting Date" <= EndDate)then begin
        ...
        // vendorLedgerEntry2.SetRange("RIE Exclude Revaluation", false); //PS001
        if VendorLedgerEntry2.Find('-')then repeat TempVendorLedgerEntry."Entry No.":=VendorLedgerEntry2."Entry No.";
                if TempVendorLedgerEntry.Insert()then;
    ...
    begin
        ...
        if VATEntry.Find('-')then repeat Accumulate(VATEntry2.Base, VATEntry.Base);
    ...
    begin
        case GLSetup."VAT Exchange Rate Adjustment" of GLSetup."VAT Exchange Rate Adjustment"::"Adjust Amount": AmountLCY:=Round(CurrExchRate2.ExchangeAmtFCYToLCYAdjmt(PostingDate, GLSetup."Additional Reporting Currency", AmountAddCurr, AddCurrCurrencyFactor));
        GLSetup."VAT Exchange Rate Adjustment"::"Adjust Additional-Currency Amount": AmountAddCurr:=Round(CurrExchRate2.ExchangeAmtLCYToFCY(PostingDate, GLSetup."Additional Reporting Currency", AmountLCY, AddCurrCurrencyFactor));
        end;
    end;
    ...
    begin
        ...
        case GLSetup."VAT Exchange Rate Adjustment" of GLSetup."VAT Exchange Rate Adjustment"::"Adjust Amount": PostGLAccAdjmt(AccNo, GLSetup."VAT Exchange Rate Adjustment"::"Adjust Amount", -AmountLCY, BaseLCY, BaseAddCurr);
        GLSetup."VAT Exchange Rate Adjustment"::"Adjust Additional-Currency Amount": PostGLAccAdjmt(AccNo, GLSetup."VAT Exchange Rate Adjustment"::"Adjust Additional-Currency Amount", -AmountAddCurr, BaseLCY, BaseAddCurr);
        end;
    end;
    ...
    begin
        if(VATEntry2.Amount <> 0) or (VATEntry2."Additional-Currency Amount" <> 0)then begin
            ...
            AdjustVATAccount(TaxJurisdiction."Tax Account (Purchases)", VATEntry2.Amount, VATEntry2."Additional-Currency Amount", VATEntryTotalBase.Amount, VATEntryTotalBase."Additional-Currency Amount");
            if UseTax then begin
                TaxJurisdiction.TestField("Reverse Charge (Purchases)");
                AdjustVATAccount(TaxJurisdiction."Reverse Charge (Purchases)", -VATEntry2.Amount, -VATEntry2."Additional-Currency Amount", -VATEntryTotalBase.Amount, -VATEntryTotalBase."Additional-Currency Amount");
            end;
        end;
        if(VATEntry2."Remaining Unrealized Amount" <> 0) or (VATEntry2."Add.-Curr. Rem. Unreal. Amount" <> 0)then begin
            TaxJurisdiction.TestField("Unrealized VAT Type");
            TaxJurisdiction.TestField("Unreal. Tax Acc. (Purchases)");
            AdjustVATAccount(TaxJurisdiction."Unreal. Tax Acc. (Purchases)", VATEntry2."Remaining Unrealized Amount", VATEntry2."Add.-Curr. Rem. Unreal. Amount", VATEntryTotalBase."Remaining Unrealized Amount", VATEntry2."Add.-Curr. Rem. Unreal. Amount");
            if UseTax then begin
                TaxJurisdiction.TestField("Unreal. Rev. Charge (Purch.)");
                AdjustVATAccount(TaxJurisdiction."Unreal. Rev. Charge (Purch.)", -VATEntry2."Remaining Unrealized Amount", -VATEntry2."Add.-Curr. Rem. Unreal. Amount", -VATEntryTotalBase."Remaining Unrealized Amount", -VATEntryTotalBase."Add.-Curr. Rem. Unreal. Amount");
            end;
        end;
    end;
    ...
    begin
        ...
        AdjustVATAccount(TaxJurisdiction."Tax Account (Sales)", VATEntry2.Amount, VATEntry2."Additional-Currency Amount", VATEntryTotalBase.Amount, VATEntryTotalBase."Additional-Currency Amount");
        if(VATEntry2."Remaining Unrealized Amount" <> 0) or (VATEntry2."Add.-Curr. Rem. Unreal. Amount" <> 0)then begin
            TaxJurisdiction.TestField("Unrealized VAT Type");
            TaxJurisdiction.TestField("Unreal. Tax Acc. (Sales)");
            AdjustVATAccount(TaxJurisdiction."Unreal. Tax Acc. (Sales)", VATEntry2."Remaining Unrealized Amount", VATEntry2."Add.-Curr. Rem. Unreal. Amount", VATEntryTotalBase."Remaining Unrealized Amount", VATEntryTotalBase."Add.-Curr. Rem. Unreal. Amount");
        end;
    end;
    ...
    begin
        GenJnlLine.Init();
        case ExchRateAdjmt of "G/L Account"."Exchange Rate Adjustment"::"Adjust Amount": begin
        ...
        "G/L Account"."Exchange Rate Adjustment"::"Adjust Additional-Currency Amount": begin
        ...
        if GenJnlLine.Amount <> 0 then begin
            ...
            case GenJnlLine."Additional-Currency Posting" of GenJnlLine."Additional-Currency Posting"::"Amount Only": GenJnlLine.Description:=StrSubstNo(PostingDescription, GLSetup."Additional Reporting Currency", AddCurrNetChange);
            GenJnlLine."Additional-Currency Posting"::"Additional-Currency Amount Only": GenJnlLine.Description:=StrSubstNo(PostingDescription, '', NetChange);
            end;
            GenJnlLine."System-Created Entry":=true;
            GenJnlLine."Source Code":=SourceCodeSetup."Exchange Rate Adjmt.";
    ...
    begin
        ...
        if GLAmtTotal <> 0 then begin
            if GLAmtTotal < 0 then GenJnlLine."Account No.":=Currency3.GetRealizedGLLossesAccount()
            else
                GenJnlLine."Account No.":=Currency3.GetRealizedGLGainsAccount();
            GenJnlLine.Description:=StrSubstNo(PostingDescription, GLSetup."Additional Reporting Currency", GLAddCurrNetChangeTotal);
            GenJnlLine."Additional-Currency Posting":=GenJnlLine."Additional-Currency Posting"::"Amount Only";
            GenJnlLine."Currency Code":='';
        ...
        if GLAddCurrAmtTotal <> 0 then begin
            if GLAddCurrAmtTotal < 0 then GenJnlLine."Account No.":=Currency3.GetRealizedGLLossesAccount()
            else
                GenJnlLine."Account No.":=Currency3.GetRealizedGLGainsAccount();
            GenJnlLine.Description:=StrSubstNo(PostingDescription, '', GLNetChangeTotal);
            GenJnlLine."Additional-Currency Posting":=GenJnlLine."Additional-Currency Posting"::"Additional-Currency Amount Only";
            GenJnlLine."Currency Code":=GLSetup."Additional Reporting Currency";
    ...
    begin
        ...
        if GLAcc."Exchange Rate Adjustment" <> GLAcc."Exchange Rate Adjustment"::"No Adjustment" then begin
            ...
            Error(Text017Err, GLAcc.FieldCaption("Exchange Rate Adjustment"), GLAcc.TableCaption(), GLAcc."No.", GLAcc."Exchange Rate Adjustment", SetupTableName, GLSetup.FieldCaption("VAT Exchange Rate Adjustment"), GLSetup.TableCaption(), SetupFieldName);
        end;
    end;
    local procedure HandleCustDebitCredit(Correction: Boolean; AdjAmount: Decimal)
    begin
        if(AdjAmount > 0) and not Correction or (AdjAmount < 0) and Correction then begin
    ...
    begin
        case GenJnlLine."Account Type" of GenJnlLine."Account Type"::"G/L Account": DimMgt.AddDimSource(DefaultDimSource, Database::"G/L Account", GenJnlLine."Account No.");
        GenJnlLine."Account Type"::"Bank Account": DimMgt.AddDimSource(DefaultDimSource, Database::"Bank Account", GenJnlLine."Account No.");
        end;
        DimSetID:=DimMgt.GetDefaultDimID(DefaultDimSource, GenJnlLine."Source Code", GenJnlLine."Shortcut Dimension 1 Code", GenJnlLine."Shortcut Dimension 2 Code", GenJnlLine."Dimension Set ID", 0);
    ...
    begin
        if DimSetEntry.Find('-')then repeat DimBuf."Table ID":=DATABASE::"Dimension Buffer";
                DimBuf."Entry No.":=0;
    ...
    local procedure GetDimCombID(var DimBuf: Record "Dimension Buffer"): Integer var
        DimEntryNo: Integer;
    begin
        DimEntryNo:=DimBufMgt.FindDimensions(DimBuf);
        if DimEntryNo = 0 then DimEntryNo:=DimBufMgt.InsertDimensions(DimBuf);
        exit(DimEntryNo);
    end;
    local procedure PostGenJnlLine(var GenJnlLine: Record "Gen. Journal Line"; var DimSetEntry: Record "Dimension Set Entry")Result: Integer var
        IsHandled: Boolean;
    begin
        IsHandled:=false;
    ...
    local procedure GetGlobalDimVal(GlobalDimCode: Code[20]; var DimSetEntry: Record "Dimension Set Entry"): Code[20]var
        DimVal: Code[20];
    begin
        if GlobalDimCode = '' then DimVal:=''
        else
        begin
            DimSetEntry.SetRange("Dimension Code", GlobalDimCode);
            if DimSetEntry.Find('-')then DimVal:=DimSetEntry."Dimension Value Code"
            else
                DimVal:='';
    ...
    begin
        if PostingDate < StartDate then Error(Text018Err);
        if PostingDate > EndDateReq then Error(Text018Err);
    end;
    ...
    begin
        ...
        CustLedgerEntry.CalcFields(Amount, "Amount (LCY)", "Remaining Amount", "Remaining Amt. (LCY)", "Original Amt. (LCY)", "Debit Amount", "Credit Amount", "Debit Amount (LCY)", "Credit Amount (LCY)");
        ...
        CurrAdjAmount:=Round(CurrExchRate.ExchangeAmtFCYToLCYAdjmt(PostingDate2, TempCurrencyToAdjust.Code, CustLedgerEntry."Remaining Amount", TempCurrencyToAdjust."Currency Factor")) - CustLedgerEntry."Remaining Amt. (LCY)";
        if CurrAdjAmount <> 0 then begin
            ...
            Correction:=(CustLedgerEntry."Debit Amount" < 0) or (CustLedgerEntry."Credit Amount" < 0) or (CustLedgerEntry."Debit Amount (LCY)" < 0) or (CustLedgerEntry."Credit Amount (LCY)" < 0);
            if OldAdjAmount > 0 then case true of(CurrAdjAmount > 0): begin
                ...
                (CurrAdjAmount < 0): if -CurrAdjAmount <= OldAdjAmount then begin
                        TempDtldCustLedgEntry."Amount (LCY)":=CurrAdjAmount;
                    ...
                    begin
                        ...
                        InsertTempDtldCustomerLedgerEntry();
                        NewEntryNo:=NewEntryNo + 1;
                        AdjExchRateBufIndex:=AdjExchRateBufferUpdate(CustLedgerEntry."Currency Code", CustLedgerEntry."Customer Posting Group", 0, 0, -OldAdjAmount, 0, -OldAdjAmount, DimEntryNo, PostingDate2, Customer."IC Partner Code");
            ...
            if OldAdjAmount < 0 then case true of(CurrAdjAmount < 0): begin
                ...
                (CurrAdjAmount > 0): if CurrAdjAmount <= -OldAdjAmount then begin
                    ...
                    begin
                        ...
                        InsertTempDtldCustomerLedgerEntry();
                        NewEntryNo:=NewEntryNo + 1;
                        AdjExchRateBufIndex:=AdjExchRateBufferUpdate(CustLedgerEntry."Currency Code", CustLedgerEntry."Customer Posting Group", 0, 0, -OldAdjAmount, -OldAdjAmount, 0, DimEntryNo, PostingDate2, Customer."IC Partner Code");
                        TempDtldCustLedgEntry."Transaction No.":=AdjExchRateBufIndex;
                        ModifyTempDtldCustomerLedgerEntry();
                        Adjust:=false;
                    end;
            ...
            if not Adjust then begin
                ...
                else if CurrAdjAmount > 0 then begin
                        TempDtldCustLedgEntry."Entry Type":=TempDtldCustLedgEntry."Entry Type"::"Unrealized Gain";
                        GainsAmount:=CurrAdjAmount;
                        LossesAmount:=0;
                    end;
            ...
            TotalAdjAmount:=TotalAdjAmount + CurrAdjAmount;
            if not HideUI then Window.Update(4, TotalAdjAmount);
            AdjExchRateBufIndex:=AdjExchRateBufferUpdate(CustLedgerEntry."Currency Code", CustLedgerEntry."Customer Posting Group", CustLedgerEntry."Remaining Amount", CustLedgerEntry."Remaining Amt. (LCY)", TempDtldCustLedgEntry."Amount (LCY)", GainsAmount, LossesAmount, DimEntryNo, PostingDate2, Customer."IC Partner Code");
            TempDtldCustLedgEntry."Transaction No.":=AdjExchRateBufIndex;
            ModifyTempDtldCustomerLedgerEntry();
        end;
    end;
    ...
    begin
        ...
        CopyDimSetEntryToDimBuf(DimSetEntry, TempDimBuf);
        DimEntryNo:=GetDimCombID(TempDimBuf);
        VendLedgerEntry.CalcFields(Amount, "Amount (LCY)", "Remaining Amount", "Remaining Amt. (LCY)", "Original Amt. (LCY)", "Debit Amount", "Credit Amount", "Debit Amount (LCY)", "Credit Amount (LCY)");
        ...
        // Calculate New Unrealized Gains and Losses
        CurrAdjAmount:=Round(CurrExchRate.ExchangeAmtFCYToLCYAdjmt(PostingDate2, TempCurrencyToAdjust.Code, VendLedgerEntry."Remaining Amount", TempCurrencyToAdjust."Currency Factor")) - VendLedgerEntry."Remaining Amt. (LCY)";
        if CurrAdjAmount <> 0 then begin
            ...
            Correction:=(VendLedgerEntry."Debit Amount" < 0) or (VendLedgerEntry."Credit Amount" < 0) or (VendLedgerEntry."Debit Amount (LCY)" < 0) or (VendLedgerEntry."Credit Amount (LCY)" < 0);
            if OldAdjAmount > 0 then case true of(CurrAdjAmount > 0): begin
                ...
                (CurrAdjAmount < 0): if -CurrAdjAmount <= OldAdjAmount then begin
                    ...
                    begin
                        ...
                        InsertTempDtldVendorLedgerEntry();
                        NewEntryNo:=NewEntryNo + 1;
                        AdjExchRateBufIndex:=AdjExchRateBufferUpdate(VendLedgerEntry."Currency Code", VendLedgerEntry."Vendor Posting Group", 0, 0, -OldAdjAmount, 0, -OldAdjAmount, DimEntryNo, PostingDate2, Vendor."IC Partner Code");
            ...
            if OldAdjAmount < 0 then case true of(CurrAdjAmount < 0): begin
                ...
                (CurrAdjAmount > 0): if CurrAdjAmount <= -OldAdjAmount then begin
                    ...
                    begin
                        ...
                        InsertTempDtldVendorLedgerEntry();
                        NewEntryNo:=NewEntryNo + 1;
                        AdjExchRateBufIndex:=AdjExchRateBufferUpdate(VendLedgerEntry."Currency Code", VendLedgerEntry."Vendor Posting Group", 0, 0, -OldAdjAmount, -OldAdjAmount, 0, DimEntryNo, PostingDate2, Vendor."IC Partner Code");
                        TempDtldVendLedgEntry."Transaction No.":=AdjExchRateBufIndex;
                        ModifyTempDtldVendorLedgerEntry();
                        Adjust:=false;
                    end;
            ...
            TotalAdjAmount:=TotalAdjAmount + CurrAdjAmount;
            if not HideUI then Window.Update(4, TotalAdjAmount);
            AdjExchRateBufIndex:=AdjExchRateBufferUpdate(VendLedgerEntry."Currency Code", VendLedgerEntry."Vendor Posting Group", VendLedgerEntry."Remaining Amount", VendLedgerEntry."Remaining Amt. (LCY)", TempDtldVendLedgEntry."Amount (LCY)", GainsAmount, LossesAmount, DimEntryNo, PostingDate2, Vendor."IC Partner Code");
            TempDtldVendLedgEntry."Transaction No.":=AdjExchRateBufIndex;
            ModifyTempDtldVendorLedgerEntry();
        end;
    end;
    ...
    begin
        ...
        if TempCustLedgerEntry.FindSet()then repeat CustLedgerEntry2.Get(TempCustLedgerEntry."Entry No.");
                CustLedgerEntry2.SetRange("Date Filter", 0D, PostingDate2);
                CustLedgerEntry2.CalcFields("Remaining Amount", "Remaining Amt. (LCY)");
                if ShouldAdjustEntry(PostingDate2, CustLedgerEntry2."Currency Code", CustLedgerEntry2."Remaining Amount", CustLedgerEntry2."Remaining Amt. (LCY)", CustLedgerEntry2."Adjusted Currency Factor")then begin
                    ...
                    if DetailedCustLedgEntry.FindSet()then repeat AdjustCustomerLedgerEntry(CustLedgerEntry2, DetailedCustLedgEntry."Posting Date");
    ...
    begin
        PostingDate2:=GenJournalLine."Posting Date";
        if TempVendLedgerEntry.FindSet()then repeat VendLedgerEntry2.Get(TempVendLedgerEntry."Entry No.");
                VendLedgerEntry2.SetRange("Date Filter", 0D, PostingDate2);
                VendLedgerEntry2.CalcFields("Remaining Amount", "Remaining Amt. (LCY)");
                if ShouldAdjustEntry(PostingDate2, VendLedgerEntry2."Currency Code", VendLedgerEntry2."Remaining Amount", VendLedgerEntry2."Remaining Amt. (LCY)", VendLedgerEntry2."Adjusted Currency Factor")then begin
                    InitVariablesForSetLedgEntry(GenJournalLine);
                    ...
                    if DetailedVendLedgEntry.FindSet()then repeat AdjustVendorLedgerEntry(VendLedgerEntry2, DetailedVendLedgEntry."Posting Date");
    ...
    local procedure ShouldAdjustEntry(PostingDate: Date; CurCode: Code[10]; RemainingAmount: Decimal; RemainingAmtLCY: Decimal; AdjCurFactor: Decimal): Boolean begin
    ...
    begin
        InitializeRequest(GenJournalLine."Posting Date", GenJournalLine."Posting Date", Text016Txt, GenJournalLine."Posting Date");
    ...
    local procedure GetUnrealizedGainsAccount(Currency: Record Currency)AccountNo: Code[20]var
        IsHandled: Boolean;
    begin
        IsHandled:=false;
    ...
    local procedure GetUnrealizedLossesAccount(Currency: Record Currency)AccountNo: Code[20]var
        IsHandled: Boolean;
    begin
        IsHandled:=false;
    ...
    //PS001 Start
    local procedure InsertGLEntries(var TempDtldgLEntry: Record "Detailed g/l Entry PB" temporary; var TempDtldCVLedgEntryBuf: Record "Detailed CV Ledg. Entry Buffer" temporary)
    var
        DtldgLEntry2: Record "Detailed g/l Entry PB";
        GLEntry: Record "G/L Entry";
        LastEntryNo: Integer;
        LastTransactionNo: Integer;
    begin
        GLEntry.GetLastEntry(LastEntryNo, LastTransactionNo);
        if TempDtldgLEntry.Find('-')then repeat if TempDtldCVLedgEntryBuf.Get(TempDtldCustLedgEntry."Transaction No.")then TempDtldgLEntry."Transaction No.":=TempDtldCVLedgEntryBuf."Transaction No."
                else
                    TempDtldgLEntry."Transaction No.":=LastTransactionNo;
                DtldgLEntry2:=TempDtldgLEntry;
                //DtldgLEntry2."Exch. Rate Adjmt. Reg. No." := TempDtldCVLedgEntryBuf."Exch. Rate Adjmt. Reg. No.";
                DtldgLEntry2."Entry No.":=FindLastDtldGLEntryNo() + 1; //temprajan
                DtldgLEntry2.Insert(true);
            until TempDtldgLEntry.Next() = 0;
    end;
    local procedure FindLastDtldGLEntryNo()DtldGLEntryNo: Integer var
        DetailedGLEntryCZA: Record "Detailed G/L Entry PB";
    begin
        if DetailedGLEntryCZA.FindLast()then DtldGLEntryNo:=DetailedGLEntryCZA."Entry No."
        else
            DtldGLEntryNo:=0;
    end;
    local procedure PostGLAdjmt(AdjExchRateBuffer: Record "Adjust Exchange Rate Buffer"; var TempDtldCVLedgEntryBuf: Record "Detailed CV Ledg. Entry Buffer" temporary; var TempDimSetEntry: Record "Dimension Set Entry" temporary)
    var
        CustPostingGr: Record "Customer Posting Group";
        GLSetup: Record "General Ledger Setup";
    begin
        ...
        //CustPostingGr.Get(TempAdjExchRateBuffer."Posting Group");
        GLSetup.Get();
        TempDtldCVLedgEntryBuf."Transaction No.":=PostAdjmtNew(AdjExchRateBuffer."Account No.", AdjExchRateBuffer.AdjAmount, AdjExchRateBuffer.AdjBase, AdjExchRateBuffer."Currency Code", TempDimSetEntry, AdjExchRateBuffer."Posting Date", AdjExchRateBuffer."IC Partner Code");
        //OnAfterPostCustAdjmt(AdjExchRateBuffer);
        if TempDtldCVLedgEntryBuf.Insert()then;
        InsertExchRateAdjmtReg("Exch. Rate Adjmt. Account Type"::"G/L Account", AdjExchRateBuffer."Posting Group", AdjExchRateBuffer."Currency Code");
        TempDtldCVLedgEntryBuf."Exch. Rate Adjmt. Reg. No.":=ExchRateAdjReg."No.";
        TempDtldCVLedgEntryBuf.Modify();
        TotalRevalAccAdjusted+=1;
    end;
    local procedure PrepareTempGLEntry(GLAcc: Record "g/l account"; var TempgLEntry: Record "g/l entry" temporary)
    var
    ...
    begin
        ...
        Currency.CopyFilter(Code, gLEntry2."Original Currency PB");
        GLSetup.Get();
        gLEntry2.SetRange("g/l account No.", glacc."No.");
        gLEntry2.SetRange("Closed PB", false);
        if Currency.GetFilter(code) = '' then gLEntry2.SetFilter("original Currency PB", '<>%1', '');
        if StartDate <> 0D then gLEntry2.SetRange("Posting Date", startdate, EndDate)
        else
            gLEntry2.SetRange("Posting Date", 0D, EndDate);
        //gLEntry2.SetRange("Include Revaluation", true);
        GLSetup.Get();
        //temporary code commented >>
        gLEntry2.SetFilter("Source Code", '<>%1', GLSetup."RIE Unappl. G/L Entries Appln");
        gLEntry2.Setrange(Reversed, false); //Shanky - Added on 23july25
        //temporary code commented <<
        //gLEntry2.SetFilter("Source Code", '<>%1', 'EXCHRATADJ');
        if gLEntry2.Find('-')then repeat TempgLEntry."Entry No.":=gLEntry2."Entry No.";
                //TempgLEntry."RIE Original Currency":=GLSetup."LCY Code";
                //TempgLEntry."RIE Original Amount":=0;
                if TempgLEntry.Insert()then;
            until gLEntry2.Next() = 0;
        gLEntry2.Reset();
    end;
    local procedure SetUnrealizedGainLossFilterGL(var DtldGLEntry: Record "Detailed G/L Entry PB"; EntryNo: Integer)
    begin
        //with DtldGLEntry do begin
        DtldGLEntry.Reset();
        DtldGLEntry.SetCurrentKey("G/L Entry No.", "Entry Type");
        DtldGLEntry.SetRange("G/L Entry No.", EntryNo);
        DtldGLEntry.SetRange("Entry Type", DtldGLEntry."Entry Type"::"Unrealized Loss", DtldGLEntry."Entry Type"::"Unrealized Gain");
    //end;
    end;
    local procedure InitDtldgLEntry(gLEntry: Record "g/l entry"; var DtldgLEntry: Record "Detailed g/l Entry PB")
    begin
        DtldgLEntry.Init();
        DtldgLEntry."g/l Entry No.":=gLEntry."Entry No.";
        DtldgLEntry.Amount:=0;
        DtldgLEntry."G/L Account No.":=gLEntry."G/L Account No.";
        //DtldCustLedgEntry."Currency Code" := CustLedgEntry."Currency Code";
        DtldgLEntry."User ID":=CopyStr(UserId(), 1, MaxStrLen(DtldgLEntry."User ID"));
    //DtldCustLedgEntry."Source Code" := SourceCodeSetup."Exchange Rate Adjmt.";
    //DtldCustLedgEntry."Journal Batch Name" := CustLedgEntry."Journal Batch Name";
    //DtldCustLedgEntry."Reason Code" := CustLedgEntry."Reason Code";
    //DtldCustLedgEntry."Initial Entry Due Date" := CustLedgEntry."Due Date";
    ...
    local procedure InsertTempDtldgLEntry()
    begin
        TempDtldgLEntry."Adjust Exchange Entry":=true;
        TempDtldgLEntry.Insert();
        TempDtldgLEntrySums:=TempDtldgLEntry;
        TempDtldgLEntrySums.Insert();
    end;
    local procedure ModifyTempDtldgLEntry()
    begin
        TempDtldgLEntry.Modify();
        TempDtldgLEntrySums:=TempDtldgLEntry;
        TempDtldgLEntrySums.Modify();
    end;
    procedure AdjustGLEntry(GLEntry: Record "G/L Entry"; PostingDate2: Date)
    var
        DimSetEntry: Record "Dimension Set Entry";
        DimEntryNo: Integer;
        OldAdjAmount: Decimal;
        ...
        Amt: Decimal;
    begin
        GLEntry.SetRange("Date Filter PB", 0D, PostingDate2);
        TempCurrencyToAdjust.Get(GLEntry."Original Currency PB");
        GainsAmount:=0;
        LossesAmount:=0;
        OldAdjAmount:=0;
        Adjust:=false;
        ...
        CopyDimSetEntryToDimBuf(DimSetEntry, TempDimBuf);
        DimEntryNo:=GetDimCombID(TempDimBuf);
        gLEntry.CalcFields("Remaining Amount PB", "Remaining Amount (LCY) PB");
        //Amt := GLEntry."RIE Original Amount";
        // Calculate Old Unrealized Gains and Losses    
        SetUnrealizedGainLossFiltergl(DtldglEntry, gLEntry."Entry No.");
        DtldgLEntry.CalcSums("Amount (LCY)");
        SetUnrealizedGainLossFiltergl(TempDtldgLEntrySums, gLEntry."Entry No.");
        TempDtldgLEntrySums.CalcSums("Amount (LCY)");
        OldAdjAmount:=DtldgLEntry."Amount (LCY)" + TempDtldgLEntrySums."Amount (LCY)";
        gLEntry."Remaining Amount (LCY) PB"+=TempDtldgLEntrySums."Amount (LCY)";
        TempDtldgLEntrySums.Reset();
        // Modify Currency factor on Customer Ledger Entry
        if glEntry."Adjusted Curr. Factor PB" <> TempCurrencyToAdjust."Currency Factor" then begin
            glEntry."Adjusted Curr. Factor PB":=TempCurrencyToAdjust."Currency Factor";
            glEntry.Modify();
        end;
        // Calculate New Unrealized Gains and Losses
        CurrAdjAmount:=Round(CurrExchRate.ExchangeAmtFCYToLCYAdjmt(PostingDate2, TempCurrencyToAdjust.Code, GLEntry."Remaining Amount PB", TempCurrencyToAdjust."Currency Factor")) - gLEntry."Remaining Amount (LCY) PB";
        IF CurrAdjAmount <> 0 then begin //Shanky- Added- 250725
            glEntry.Revalued:=true;
            glEntry.Modify();
        end;
        if CurrAdjAmount <> 0 then begin
            //OnAdjustCustomerLedgerEntryOnBeforeInitDtldCustLedgEntry(Customer, CustLedgerEntry);
            InitDtldgLEntry(gLEntry, TempDtldgLEntry);
            TempDtldgLEntry."Entry No.":=NewEntryNo;
            TempDtldgLEntry."Posting Date":=PostingDate;
            TempDtldgLEntry."Document No.":=PostingDocNo;
            Correction:=(gLEntry."Debit Amount" < 0) or (gLEntry."Credit Amount" < 0);
            if OldAdjAmount > 0 then case true of(CurrAdjAmount > 0): begin
                    TempDtldgLEntry."Amount (LCY)":=CurrAdjAmount;
                    TempDtldgLEntry."Entry Type":=TempDtldgLEntry."Entry Type"::"Unrealized Gain";
                    //HandleCustDebitCredit(Correction, TempDtldCustLedgEntry."Amount (LCY)");
                    InsertTempDtldgLentry();
                    NewEntryNo:=NewEntryNo + 1;
                    GainsAmount:=CurrAdjAmount;
                    Adjust:=true;
                end;
                (CurrAdjAmount < 0): if -CurrAdjAmount <= OldAdjAmount then begin
                        TempDtldgLEntry."Amount (LCY)":=CurrAdjAmount;
                        TempDtldgLEntry."Entry Type":=TempDtldgLEntry."Entry Type"::"Unrealized Loss";
                        //HandleCustDebitCredit(Correction, TempDtldCustLedgEntry."Amount (LCY)");
                        InsertTempDtldgLEntry();
                        NewEntryNo:=NewEntryNo + 1;
                        LossesAmount:=CurrAdjAmount;
                    ...
                    begin
                        CurrAdjAmount:=CurrAdjAmount + OldAdjAmount;
                        TempDtldgLEntry."Amount (LCY)":=-OldAdjAmount;
                        TempDtldgLEntry."Entry Type":=TempDtldgLEntry."Entry Type"::"Unrealized Loss";
                        //HandleCustDebitCredit(Correction, TempDtldCustLedgEntry."Amount (LCY)");
                        InsertTempDtldgLEntry();
                        NewEntryNo:=NewEntryNo + 1;
                        AdjExchRateBufIndex:=AdjExchRateBufferUpdatenew(gLEntry."Original Currency PB", '', 0, 0, -OldAdjAmount, 0, -OldAdjAmount, DimEntryNo, PostingDate, '', GLEntry."G/L Account No.");
                        TempDtldgLEntry."Transaction No.":=AdjExchRateBufIndex;
                        ModifyTempDtldgLEntry();
                        Adjust:=false;
                    end;
                end;
            if OldAdjAmount < 0 then case true of(CurrAdjAmount < 0): begin
                    TempDtldgLEntry."Amount (LCY)":=CurrAdjAmount;
                    TempDtldgLEntry."Entry Type":=TempDtldgLEntry."Entry Type"::"Unrealized Loss";
                    //HandleCustDebitCredit(Correction, TempDtldgLEntry.Amount);
                    InsertTempDtldgLEntry();
                    NewEntryNo:=NewEntryNo + 1;
                    LossesAmount:=CurrAdjAmount;
                    Adjust:=true;
                ...
                (CurrAdjAmount > 0): if CurrAdjAmount <= -OldAdjAmount then begin
                    ...
                    begin
                        ...
                        InsertTempDtldgLEntry();
                        NewEntryNo:=NewEntryNo + 1;
                        AdjExchRateBufIndex:=AdjExchRateBufferUpdatenew(gLEntry."Original Currency PB", '', 0, 0, -OldAdjAmount, -OldAdjAmount, 0, DimEntryNo, PostingDate, '', GLEntry."G/L Account No.");
                        TempDtldgLEntry."Transaction No.":=AdjExchRateBufIndex;
                        ModifyTempDtldgLEntry();
                        Adjust:=false;
                    end;
            ...
            TotalAdjAmount:=TotalAdjAmount + CurrAdjAmount;
            if not HideUI then Window.Update(4, TotalAdjAmount);
            AdjExchRateBufIndex:=AdjExchRateBufferUpdatenew(gLEntry."Original Currency PB", '', GLEntry."Remaining Amount PB", GLEntry."Remaining Amount (LCY) PB", TempDtldgLEntry."Amount (LCY)", GainsAmount, LossesAmount, DimEntryNo, PostingDate, '', GLEntry."G/L Account No.");
            TempDtldgLEntry."Transaction No.":=AdjExchRateBufIndex;
            ModifyTempDtldgLentry();
        end;
    end;
    //PS001 End
    // procedure SetGLAccFilter(P_COA: Code[20])
    // begin
    //     G_GLAccNoFilter := P_COA;  //Sgarg Added
    // end;
    var G_GLAccNoFilter: Code[20];
