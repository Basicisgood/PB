report 50100 "Suggest Vendor Payments PB"
{
    Caption = 'Suggest Vendor Payments PB';
    ProcessingOnly = true;
    ApplicationArea = All;

    dataset
    {
        dataitem(Company_loop; Company)
        {
            dataitem(Vendor; Vendor)
            {
                DataItemTableView = sorting(Blocked)where(Blocked=filter(=" "));
                RequestFilterFields = "No.", "Payment Method Code";
                DataItemLinkReference = Company_loop;
                ...
                begin
                    ...
                    if UsePaymentDisc and not StopPayments then begin
                        ...
                        if FindSet()then repeat Clear(VendorBalance);
                                ...
                                VendorBalance:="Balance (LCY)";
                                Window2.Update(1, "No.");
                                TempPayableVendorLedgerEntry.ChangeCompany(Company_loop.Name);
                ...
                begin
                    //NS <<
                    ChangeCompany(Company_loop.Name);
                    //NS >>
                    ...
                    //#NS <<
                    Vend2.ChangeCompany(Company_loop.Name);
                    //#NS >>
                    ...
                    Window.Open(Text006);
                    //#NS <<
                    SelectedDim.ChangeCompany(Company_loop.Name);
                    //#NS >>
            ...
            begin
                // Vendor.Reset();
                // Vendor.ChangeCompany(Company_loop.Name);
                ...
                //         end;
                //     Until Vendor.Next() = 0;
                //If NOT (Company_loop.Name = CompanyName) then
                IF ClearOrphan then;
                ClearOrphanVLE();
            end;
        }
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    // field(ClearOrphan; ClearOrphan)
                    // {
                    //     ApplicationArea = All;
                    //     Caption = 'Clear Orphan';
                    //     ToolTip = 'Specifies the value of the Clear Orphan field.';
                    // }
                    }
    ...
    var
        IsHandled: Boolean;
    begin
        VendLedgEntry.ChangeCompany(Company_loop.Name);
        VendLedgEntry.Reset();
        ...
        if Future then begin
            ...
            VendLedgEntry.SetRange("Payment Date", LastDueDateToPayReq + 1, DMY2Date(31, 12, 9999));
            //NS >>
            ...
            VendLedgEntry.SetRange("Payment Date", 0D, LastDueDateToPayReq);
        //NS >>
        ...
        IF NOT(ShortcutDim4 = '')then VendLedgEntry.SetFilter("Shortcut Dimension 3 Code_PB", ShortcutDim4);
        IF NOT(ShortcutDim5 = '')then VendLedgEntry.SetFilter("Shortcut Dimension 3 Code_PB", ShortcutDim5);
        IF NOT(ShortcutDim6 = '')then VendLedgEntry.SetFilter("Shortcut Dimension 3 Code_PB", ShortcutDim6);
        IF NOT(ShortcutDim7 = '')then VendLedgEntry.SetFilter("Shortcut Dimension 3 Code_PB", ShortcutDim7);
        IF NOT(ShortcutDim8 = '')then VendLedgEntry.SetFilter("Shortcut Dimension 3 Code_PB", ShortcutDim8);
        IF NOT(ShortcutDim9 = '')then VendLedgEntry.SetFilter("Shortcut Dimension 3 Code_PB", ShortcutDim9);
        IF NOT(ShortcutDim10 = '')then VendLedgEntry.SetFilter("Shortcut Dimension 3 Code_PB", ShortcutDim10);
        IF NOT(ShortcutDim11 = '')then VendLedgEntry.SetFilter("Shortcut Dimension 3 Code_PB", ShortcutDim11);
        IF NOT(ShortcutDim12 = '')then VendLedgEntry.SetFilter("Shortcut Dimension 3 Code_PB", ShortcutDim12);
        IF NOT(ShortcutDim13 = '')then VendLedgEntry.SetFilter("Shortcut Dimension 3 Code_PB", ShortcutDim13);
        ...
        if VendLedgEntry.FindSet()then repeat IsHandled:=false;
                ...
                if not IsHandled then begin
                    ...
                    // CODEUNIT.Run(CODEUNIT::"Global Vend. Entry-Edit", VendLedgEntry);   //Stopped application NS//
                    end;
                end;
    ...
    begin
        ...
        if PaymentToleranceMgt.CheckCalcPmtDiscGenJnlVend(GenJnlLine, VendLedgEntry, 0, false)then GenJnlLine.Amount:=-(VendLedgEntry."Remaining Amount" - VendLedgEntry.GetRemainingPmtDiscPossible(GenJnlLine."Posting Date"))
        else
            GenJnlLine.Amount:=-VendLedgEntry."Remaining Amount";
        GenJnlLine.Validate(Amount);
        // IF GenJnlLine.CurrentCompany() = Company_loop.Name then
        GenJnlLine."Company Code":=Company_loop.Name;
        ...
        TempPayableVendorLedgerEntry.Future:=(VendLedgEntry."Payment Date" > LastDueDateToPayReq);
        //NS >>
    ...
    begin
        TempVendorPaymentBuffer.ChangeCompany(Company_loop.Name);
        ...
        if TempPayableVendorLedgerEntry.Find('-')then repeat TempPayableVendorLedgerEntry.SetRange("Vendor No.", TempPayableVendorLedgerEntry."Vendor No.");
                TempPayableVendorLedgerEntry.Find('-');
                repeat VendLedgEntry.ChangeCompany(Company_loop.Name);
                    ...
                    if HandledEntry then begin
                        ...
                        if IsNotAppliedEntry(GenJnlLine, VendLedgEntry)then begin
                            ...
                            if SummarizePerVend then begin
                                ...
                                begin
                                    ...
                                    //#179 VJ 22012025>>
                                    //TempVendorPaymentBuffer."Bank Document No. Applied" := NextBankDocNo;//
                        ...
                        If CompanyName = Company_loop.Name then begin //NS
        ...
        if TempVendorPaymentBuffer.Find('-')then repeat InsertGenJournalLine();
    ...
    begin
        ...
        if SummarizePerVend then GenJnlLine."Document No.":=TempVendorPaymentBuffer."Document No."
                ...
                else if(TempVendorPaymentBuffer."Vendor No." = TempOldVendorPaymentBuffer."Vendor No.") and (TempVendorPaymentBuffer."Currency Code" = TempOldVendorPaymentBuffer."Currency Code")then GenJnlLine."Document No.":=TempOldVendorPaymentBuffer."Document No."
                    else
                    begin
                        GenJnlLine."Document No.":=NextDocNo;
                        RunIncrementDocumentNo(false);
        ...
        Vendor_l.ChangeCompany(Company_loop.Name);
        Vendor_l.Get(TempVendorPaymentBuffer."Vendor No.");
        if(Vendor_l."Pay-to Vendor No." <> '') and (Vendor_l."Pay-to Vendor No." <> GenJnlLine."Account No.")then Message(Text025, Vendor_l.TableCaption(), Vendor_l."No.", Vendor_l.FieldCaption("Pay-to Vendor No."), Vendor_l."Pay-to Vendor No.");
        ...
        End; //NS
        GenJnlLine.Description:=Vendor_l.Name;
    ...
    begin
        ...
        NextBankDocNo:=NoSeriesBatch.SimulateGetNextNo(l_GLSetup."Bank Document Nos.", GenJnlLine."Posting Date", NextBankDocNo); //#179 VJ 22012025
    end;
    ...
    begin
        if BalAccType = BalAccType::"Bank Account" then if BalAccNo <> '' then begin
                BankAcc.ChangeCompany(Company_loop.Name);
    ...
    begin
        if BalAccType = BalAccType::"Bank Account" then if BalAccNo <> '' then begin
                ...
                if BankAcc."Currency Code" <> '' then begin
                    TempPayableVendorLedgerEntry2.ChangeCompany(Company_loop.Name);
    ...
    begin
        ...
        VendorLedgerEntry.ChangeCompany(Company_loop.Name);
    ...
    begin
        if FromVendorLedgerEntry.Get(VendorLedgerEntryNo)then //NS <<
            ...
            exit(GetDueDateOrPaymentDiscountDate(FromVendorLedgerEntry."Payment Date", FromVendorLedgerEntry."Pmt. Discount Date", UsePaymentDisc and (FromVendorLedgerEntry."Original Pmt. Disc. Possible" <> 0)));
        //NS >>    
        exit(PostingDate);
    end;
    ...
    local procedure IsNotAppliedEntry(GenJournalLine: Record "Gen. Journal Line"; VendorLedgerEntry: Record "Vendor Ledger Entry"): Boolean begin
        exit(IsNotAppliedToCurrentBatchLine(GenJournalLine, VendorLedgerEntry) and IsNotAppliedToOtherBatchLine(GenJournalLine, VendorLedgerEntry));
    end;
    local procedure IsNotAppliedToCurrentBatchLine(GenJournalLine: Record "Gen. Journal Line"; VendorLedgerEntry: Record "Vendor Ledger Entry"): Boolean var
        PaymentGenJournalLine: Record "Gen. Journal Line";
    begin
        PaymentGenJournalLine.ChangeCompany(Company_loop.Name);
    ...
    local procedure IsNotAppliedToOtherBatchLine(GenJournalLine: Record "Gen. Journal Line"; VendorLedgerEntry: Record "Vendor Ledger Entry"): Boolean var
        PaymentGenJournalLine: Record "Gen. Journal Line";
    begin
    ...
    local procedure ClearOrphanVLE()
    var
        VendLedgEntry: Record "Vendor Ledger Entry";
    begin
        VendLedgEntry.ChangeCompany(Company_loop.Name);
        ...
        If VendLedgEntry.FindSet()then repeat VendLedgEntry."Applies-to ID":='';
                VendLedgEntry.Modify();
            until VendLedgEntry.Next() = 0;
    end;
}
