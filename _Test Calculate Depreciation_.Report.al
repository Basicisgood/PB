report 50110 "Test Calculate Depreciation"
{
    AdditionalSearchTerms = 'write down fixed asset';
    ApplicationArea = FixedAssets;
    Caption = 'Test Calculate Depreciation';
    ProcessingOnly = true;
    UsageCategory = Tasks;
    ...
    dataset
    {
        dataitem("Fixed Asset"; "Fixed Asset")
        {
            ...
            begin
                ...
                OnAfterCalculateDepreciation("No.", TempGenJnlLine, TempFAJnlLine, DeprAmount, NumberOfDays, DeprBookCode, DeprUntilDate, EntryAmounts, DaysInPeriod);
            ...
            begin
                ...
                if TempFAJnlLine.Find('-')then repeat FAJnlLine.Init();
                        FAJnlLine."Line No.":=0;
                ...
                if TempGenJnlLine.Find('-')then begin
                    ...
                    if DocumentNo = '' then if GenJnlLine.FindLast()then AutoDocumentNo:=GenJnlLine."Document No."
                        else
                            AutoDocumentNo:=FAJnlSetup.GetGenJnlDocumentNo(GenJnlLine, DeprUntilDate, true)
                    else
                        AutoDocumentNo:=DocumentNo;
                end;
                if TempGenJnlLine.Find('-')then repeat GenJnlLine.Init();
    ...
    begin
        ...
        if GenJnlLineCreatedCount > 0 then begin
            ...
            if not IsHandled then if ConfirmMgt.GetResponse(StrSubstNo(CompletionStatsGenJnlQst, GenJnlLineCreatedCount), true)then begin
    ...
    var
        DepLog: record "Depr. Calc. Log";
    begin
        DepLog.Reset();
        DepLog.SetRange("Company Code", CompanyName);
        if deplog.FindLast()then begin
            DeprBookCode:=DepLog.DeprBookCode;
            //DeprUntilDate := DepLog.;
